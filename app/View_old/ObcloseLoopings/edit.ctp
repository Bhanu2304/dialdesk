<?php
if(isset($result)){
    $cat1           =   $result['OutboundCloseLoopMaster']['Category1'];
    $cat2           =   $result['OutboundCloseLoopMaster']['Category2'];
    $cat3           =   $result['OutboundCloseLoopMaster']['Category3'];
    $cat4           =   $result['OutboundCloseLoopMaster']['Category4'];
    $cat5           =   $result['OutboundCloseLoopMaster']['Category5'];
    $closeType      =   $result['OutboundCloseLoopMaster']['close_loop'];
    $closeLabel      =   $result['OutboundCloseLoopMaster']['label'];
    $parent_id      =   $result['OutboundCloseLoopMaster']['parent_id'];
    $closeCategory  =   $result['OutboundCloseLoopMaster']['close_loop_category'];
    $closeSubCategory  =  $result['OutboundCloseLoopMaster']['close_loop_sub_category'];
    $clDate         =   $result['OutboundCloseLoopMaster']['close_looping_date'];
    $closeParentId  =   "";
    $closeId  =   $result['OutboundCloseLoopMaster']['id'];
...
function matchEditData(pid,id,cid){
    ...
    $.post("<?php echo $this->webroot?>ObcloseLoopings/editSelectCategory",{parent_id:pid,cid:cid,divid:n,CampaignId:'<?php echo isset($CampaignId)?$CampaignId:"";?>'},function(data){ 
...
function editCategory(pid,id){
    ...
    $.post("<?php echo $this->webroot?>ObcloseLoopings/selectCategory",{parent_id:pcid,divid:k,CampaignId:'<?php echo isset($CampaignId)?$CampaignId:"";?>'},function(data){
        if(pid.value ==="" && id ==="1"){
           $("#editCategory3").html('');
           $("#editCategory4").html('');
           $("#editCategory5").html('');
...
function showEditParentBycloseLoop(){
    $.post("<?php echo $this->webroot?>ObcloseLoopings/get_parent_action",{close_loop:$("#edit_close_loop").val(),CampaignId:'<?php echo isset($CampaignId)?$CampaignId:"";?>'},function(data){
...
$(document).ready(function(){
    ...
    <?php if(isset($closeLabel) && $closeLabel =='2'){?>
        $.post("<?php echo $this->webroot?>ObcloseLoopings/edit_parent_action",{close_loop:$("#edit_close_loop").val(),sd:<?php echo $parent_id;?>,CampaignId:'<?php echo isset($CampaignId)?$CampaignId:"";?>'},function(data){
            $("#edit_close_loop_category_div").show();
            $("#edit_parent_category").html(data);
        });
        $("#edit_parent_div").show();
...
<?php echo $this->Form->create('ObcloseLoopings',array('action'=>'update_close_loop','class'=>"form-horizontal row-border")); ?>
    <input type="hidden" name="id" value="<?php echo isset($closeId)?$closeId:"";?>" >
    <input type="hidden" name="CampaignId" value='<?php echo isset($CampaignId)?$CampaignId:"";?>'>
        ...
        <div class="modal-footer">
            ...
            <button type="button" onclick="return updateCloseLoop(this.form,'<?php echo $this->webroot;?>ObcloseLoopings/update_close_loop')" class="btn-web btn">Submit</button>
        </div>
