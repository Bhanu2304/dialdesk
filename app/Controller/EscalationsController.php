...
class EscalationsController extends AppController{
    ...
    public $uses=array('Escalation','EscalationValue','ClientCategory','LogincreationMaster','FieldCreation','Crone','RegistrationMaster','Matrix','SMSText','TimePeriod','Template','AgentWhatsapp');
	
    public function beforeFilter() {
        parent::beforeFilter();
        if($this->Session->check('companyid'))
        {
            $this->Auth->allow('index','add','view','view_value','delete','getParent','view_fields','view_fields2','matrix','getEcr','smsText','smsTextCustomer','time_period','save_alert_esclation','delete_matrix','save_customer_smstext','save_customer_whasaaptext','delete_sms','save_smstext','view_edit_data','update_alert_esclation','get_edit_ecr','update_customer_smstext','save_agent_alert','delete_whatsapp_caller');
        }
        else
        {
            $this->Auth->deny('index','add','view','view_value','delete','getParent','view_fields','matrix','getEcr','smsText','smsTextCustomer','time_period','save_alert_esclation','delete_matrix','save_customer_smstext','save_customer_whasaaptext','delete_sms','save_smstext','view_edit_data','update_alert_esclation','get_edit_ecr','update_customer_smstext','save_agent_alert','delete_whatsapp_caller');
        }
    }
...
public function index() {
    ...
    public function view_fields(){
        ...
        $this->set('data5',$this->Template->find('all',array('conditions'=>array('client_id'=>$ClientId))));  
    ...
    public function save_customer_whasaaptext(){
        if($this->request->is('POST')){
            ...
            $data1['sendType'] = '0';
            $data1['createdate'] = date('Y-m-d H:i:s');
            $data1['createdby'] = $ClientId;

            
            $this->Template->save($data1);
            $this->Session->setFlash("SMS Whatsapp Define SuccessFully.");
            return $this->redirect(array('controller'=>'Escalations','action'=>'view_fields','?'=>array('tab'=>$data['tabType'])));
        }  
    }

    function delete_whatsapp_caller()
    {
        $id=$this->request->query['id'];
        $this->Template->deleteAll(array('id'=>$id,'client_id' => $this->Session->read('companyid')));
