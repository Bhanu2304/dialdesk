    <?php
class DialdeeViewsController extends AppController 
{
...
public function beforeFilter()
{
    ...
    $this->DialdeeSharedRegistrationMaster->useDbConfig = 'dbDaildeeShared';
...
{
    return $this->CostCenterMaster->query("select * from cost_master cm where dialdee_client_id='$clientId' and dialdee_type='shared' limit 1");
}
...
public function addon_credit() 
{
        ...
        foreach($client_det_arr as $clr)
        {
            ...
            foreach($cost_cat_arr as $cost)
            {
                ...
                foreach($months_list1 as $monther)
                {
                    ...
                    $sel_billed_todateqry = "select ti.* from tbl_invoice ti where ti.bill_no!='' and ti.category in ('subscription-tool','Topup-Tool','Talk-Time-Tool')  and ti.status='0' and cost_center='$cost_center' and (  ti.invoiceDate >= '$date_start' AND ti.invoiceDate < '$date_end')  "; #and bill_no!=''
    ...
    $client =$this->DialdeeSharedRegistrationMaster->find('list',array('fields'=>array("company_id","company_name"),'conditions'=>"`status` != 'CL'",'order'=>array('Company_name'=>'asc')));
        ...
        foreach($client_det_arr as $clr)
        {
            ...
            foreach($cost_cat_arr as $cost)
            {
                ...
                foreach($months_list1 as $monther)
                {
                    ...
                    $free_chat = $balance_credit;
                    $sel_billed_todateqry = "select ti.* from tbl_invoice ti where ti.bill_no!='' and ti.category in ('subscription-tool','Topup-Tool','Talk-Time-Tool')  and ti.status='0' and cost_center='$cost_center' and ti.invoiceDate>='$start_date' and ti.invoiceDate<'$from_date' and (  ti.invoiceDate >= '$date_start' AND ti.invoiceDate < '$date_end')  "; #and bill_no!=''
...
public function ajax_client_wallet() 
{
    ...
    //$today_date = date('Y-03-01');
       ...
       //echo $client_qry;exit;
        ...
        echo json_encode(array('wallet'=>'0.00','free_chat'=>'0','wallet_expires_on'=>'Plan or Cost Center Not Created.')); exit;
