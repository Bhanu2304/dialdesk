...
class InitialInvoicesController extends AppController 
{
    public $uses=array('RegistrationMaster','InitialInvoice','BillMaster','InitialInvoiceTmp','Addclient','Addbranch','Addcompany','CostCenterMaster',
        ...
        'Provision','PONumber','NotificationMaster','ProvisionPart','ProvisionPartDed','CostCenterMaster',
        'BalanceMaster','PlanMaster','SendInformation','Sms_link_transaction','InvServiceMaster');
...
public function beforeFilter()
{
    ...
    $this->InvServiceMaster->useDbConfig = 'dbNorth';
    //$this->User->useDbConfig = 'dbNorth';
...
public function send_payment_data()
{
    ...
    {        
        ...
        $merchantId = 'M22CM6VMKYQKL';
        $salt_key = 'f0bcd558-6908-4eb6-b2e0-e9b704e7d222'; //production
        $url = "https://api.phonepe.com/apis/hermes$end_point"; //production
        
        //$merchantId = 'PGTESTPAYUAT'; //testing
        //$salt_key = '099eb0cd-02cf-4e2a-8aca-3e6c6aff0399'; //testing
        //$url = "https://api-preprod.phonepe.com/apis/pg-sandbox$end_point"; //testing
        $merchantUserId = $userid;
        $salt_index='1';
        $salt_url = "###$salt_index";
        $redirectUrl = 'https://dialdesk.co.in/dialdesk/Initial/paymentResponse';
        $redirectMode='POST'; //REDIRECT
        $callbackUrl='https://www.dialdesk.co.in/dialdesk/InitialInvoices/send_payment_data';
        ...
        $mobileNumber=$client_Phone;
        $amount = $amount*100;
        $merchantTransactionId='6465465abk';

        $pay_load = array();
        $pay_load['merchantId'] = $merchantId;
        $pay_load['merchantTransactionId'] = $merchantTransactionId;
        $pay_load['merchantUserId'] = $merchantUserId;
        $pay_load['amount'] = $amount;
        $pay_load['redirectUrl'] = $redirectUrl;
        $pay_load['redirectMode'] = $redirectMode;
        $pay_load['callbackUrl'] = $callbackUrl;
        $pay_load['mobileNumber'] = $mobileNumber;
        $pay_load['paymentInstrument']['type'] = 'PAY_PAGE';

        $payload_json = json_encode($pay_load);
        // print_r($payload_json);
        //echo '<br/>';
        //$payload_enc = base64_encode($payload_json);
        $payload_enc = base64_encode($payload_json);
        //echo $payload_enc;
        //echo '<br/>';
        $req = "$payload_enc$end_point$salt_key";
        //echo $req;
        //echo '<br/>';
        $req_enc = hash('sha256', $req).$salt_url;
        //echo $req_enc;
        //echo '<br/>';
        $post_req = array('request'=>$payload_enc);
         //print_r($post_req);
        //echo '<br/>';
        $post_req_json = json_encode($post_req);
        //echo $post_req_json;
        //echo '<br/>';
        $curl = curl_init();
        ...
        curl_setopt_array($curl, array(
          ...
          CURLOPT_POSTFIELDS =>$post_req_json,
        ...
        //print_r($decode_resp);exit;
        if($decode_resp['success'])
        {
            $redirectInfo = $decode_resp['data']['instrumentResponse'];
            //print_r($redirectInfo);exit;
            $pay_url = $redirectInfo['redirectInfo']['url']; 

            require_once(APP . 'Lib' . DS . 'send_email' . DS . 'function.php');

            $to=array('Email'=>$client_Email,'Name'=>$client_Email);	
            $from=array('Email'=>"ispark@teammas.in",'Name'=>'Mas Callnet');
            $reply=array('Email'=>"ispark@teammas.in",'Name'=>'Mas Callnet');
            $Sub = "Payment-Url-Link";

            $EmailText = "Hi ".$clientName.", <br> ISPARK DATACONNECT PRIVATE LIMITED has requested you to pay INR ".round($amount,2);
            $EmailText .= "<br><strong><b style=color:#FF0000>Please click on below URL and initiate the payment.</b></strong>";
            $EmailText .= "<br><strong>Payment URL : </strong>".$pay_url; 
        ...
        {
            $this->Session->setFlash("Request has been Failed. Error: ".$decode_resp['code']);
        }
...
public function add_bill() 
{
    ...
    if($result['bill_type']=='talk')
    {
        ...
        $data['AddInvParticular']['service_id'] = 8;
        $data['AddInvParticular']['service'] = 'Dialdesk Exposure (Usage over plan minutes)';
    ...
    else
    {
        ...
        $data['AddInvParticular']['qty'] = 1;
        $data['AddInvParticular']['amount'] = round($rate,2);
        $data['AddInvParticular']['service_id'] = 2;
        $data['AddInvParticular']['service'] = 'Daildesk Dedicated (Subscription)';
...
public function add_part1() 
{
    ...
    $service_Det = $this->InvServiceMaster->find('first',array('conditions'=>"id='$service_id'"));
    $service = $service_Det['InvServiceMaster']['service'];
    $sub_category = $service_Det['InvServiceMaster']['category'];
    
    $result['service_id'] = addslashes($service_id);
    $result['sub_category'] = addslashes($sub_category);
    $result['service'] = addslashes($service);
...
public function add_part() 
{
    ...
    $service_id = $this->params->query['service_id'];
    $service_Det = $this->InvServiceMaster->find('first',array('conditions'=>"id='$service_id'"));
    $service = $service_Det['InvServiceMaster']['service'];
    $sub_category = $service_Det['InvServiceMaster']['category'];
...
public function add() 
{
   ...
   $invDate=implode("-",$invDate);
    ...
    $service_list = $this->InvServiceMaster->find('list',array('fields'=>array('id','service'),'order'=>array('service'=>'asc'),"conditions"=>"status='1'"));
    ...
    {		
       ...
       // print_r($particular); exit;
        ...
        {   
            ...
            $service = $service_Det['InvServiceMaster']['service'];
            $sub_category = $service_Det['InvServiceMaster']['category'];
            $dataX['service_id']="'".addslashes($service_id)."'";
            $dataX['service']="'".addslashes($service)."'";
            $dataX['sub_category']="'".addslashes($sub_category)."'";
...
public function get_view_approved()
{
        ...
        $result = $this->params->query;
        $branch_name = $result['branch_name'];
        $qry = "select InitialInvoice.*,cm.client from tbl_invoice InitialInvoice inner join cost_master cm on InitialInvoice.cost_center=cm.cost_center where finance_year in ('2021-22','2022-23','2023-24','2024-25') and bill_no!='' and status='0'  and branch_name='$branch_name'";
        $this->set('tbl_invoice', $this->InitialInvoice->query($qry));
}
