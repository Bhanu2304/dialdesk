...
class InitialInvoicesController extends AppController 
{
    public $uses=array('RegistrationMaster','InitialInvoice','BillMaster','InitialInvoiceTmp','Addclient','Addbranch','Addcompany','CostCenterMaster',
        ...
        'Provision','PONumber','NotificationMaster','ProvisionPart','ProvisionPartDed','CostCenterMaster','BalanceMaster','PlanMaster','SendInformation','Sms_link_transaction');
...
{
    parent::beforeFilter();
    $this->Auth->allow('view_proforma_pdf','view_pdf'); 
            ...
            'reject_invoice','update_invoice','update_po','update_grn','billApproval','view_pdfgrn','view_pdfgrn1','view_pdfgrn2','get_costcenter','get_gst_type',
                    'get_service_no','view_status_change_request','delete_invoice','get_provision_months','send_payment_data','paymentResponse','add_bill_tool');
...
public function encrypt3($text, $key, $type)
	{ 
            $iv = "0123456789abcdef"; 
            $size =16;
...
public function decrypt3($crypt, $key, $type)
	{
	$iv = "0123456789abcdef";
...
public function send_payment_data()
{
    $response_url ="https://dialdesk.co.in/dialdesk/Initial/paymentResponse";

    if(isset($_POST['buy']))
    {        
        $category =   $this->request->data['category']; 
        $clientName  =   $this->request->data['client_name']; 
        $client_Email =   $this->request->data['client_email']; 
        $client_Phone  =   $this->request->data['phone_no']; 
        $amount       =   $this->request->data['amount']; 
        $sms_send_time =   date('Y-m-d h:i:s'); 
        $status      =   "Pending"; 
        $bill_no     =   'Product Test'; 
        $send_by     =   $this->Session->read("admin_id");
        $created_date =   date('Y-m-d h:i:s'); 
        $recharge_type = $this->request->data['recharge_type']; 
        $ord_no = "DD".date('Ymdhis');
        ...
        $dataX = array(
            ...
            'recharge_type' => $recharge_type,
                            ...
                            //print_r($dataX);exit;
        $this->SendInformation -> save($dataX);

        App::import('Vendor', 'JWT');

        #$URL ="https://www.avantgardepayments.com/agadmin/api/signUpLogin/agId/paygate";
        $URL ="https://uatportal.safexpay.com/api_signUpLogin_agId/paygate";
        ...
        #$key="eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJlbmNyeXB0VGV4dCI6IjVhODgzNmRjLTUwMzgtNGVlNi05NjdkLTVmMjcxNzgzNGY4OSJ9.pXPtH0xCu5b3EDFAAYU_HIfOW9mgVvwE_QGmP4D7IkI";
        $secret_key="eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJlbmNyeXB0VGV4dCI6IjVhODgzNmRjLTUwMzgtNGVlNi05NjdkLTVmMjcxNzgzNGY4OSJ9.pXPtH0xCu5b3EDFAAYU_HIfOW9mgVvwE_QGmP4D7IkI";

        //$key="pxEIPNVQg3LCIZUdWx4pNQ4bItLydFrEJ+8ajhjUfEY=";
        echo "login_auth=> ";        
        echo $login_credentials=json_encode($login);
        echo '<br/><br/>';
        $login_array=array();
        //echo 'login encypted text=>';
        $login_array['encryptText']=$login_credentials;
         //'<br/><br/>';
        ...
        echo 'login encrypt text=> ';
        $login_en_text=JWT :: encode($login_array,$secret_key);
        $login_request=$login_en_text;
        $logininit['loginRequest']=$login_request; 
        echo $val2=json_encode($logininit);
        echo '<br/><br/>';
        ...
        curl_setopt($curl,CURLOPT_POSTFIELDS,$val2);
        curl_setopt($curl,CURLOPT_RETURNTRANSFER,true);
        curl_setopt($curl, CURLOPT_HTTPHEADER, array(
        'Content-Type: application/json'));

        $test1=curl_getinfo($curl);
        //   echo "<pre>";
        //   print_r($test1);die;
        ...
        #print_r($decoded['responseData']['payLoad']);die;
        $PAYLOAD= $decoded['responseData']["payLoad"];
        //print_r($PAYLOAD);
        //echo '<br/>';
        $decrypt = JWT::decode($PAYLOAD, $secret_key,array('HS256'));
        //print_r($decrypt);
        //echo '<br/>';
        $arr=json_decode(json_encode($decrypt),true);
        #print_r($arr);die;
        $arry=$arr["encryptText"];
        $second=json_decode($arry,true);
        ...
        //  print_r($second);
        //  echo "</pre>"; exit;
       // echo 'unique login key=> ';                            
       // echo $uniqueLoginKey=$second['merchantKey'];
       // echo '<br/><br/>';
        echo 'session key=> ';
        echo $session_key=$second['sessionKey'];
        echo '<br/><br/>';
        ...
        'mobileNo'=>$client_Phone,
        'status'=>'A',
        'createdBy'=>'mayuresh',
        'successURL'=>$response_url,
        'failureURL'=>$response_url
        );
        ...
        /*$encryptText ='{
            ...
            "lastName": "Jha",
            "mediaType": "EMAIL",
            "orderId": "4h5hhh67eiuds",
            "amount": "10",
            "product": "Game",
            "expiryDate": "2024-01-15",
            "country": "IND",
            "currency": "INR",
            "customereMail": "rajkumarj@safexpay.com",
            "mobileNo": "9967756911",
            "status": " A",
            "createdBy": "Rajkumar Jha",
            "successURL": "",
            "failureURL": ""
          }';*/
        ...
        echo "request => ";
        $OrderAPI_json=json_encode($OrderAPI);
        print_r($OrderAPI_json);echo '<br/><br/>';
        #print_r($value4);die;


        #print_r($value4);exit;
        echo 'merchant key=>';
        echo $merchant_key = "wx1wXj33fOOdasS1/S+4AW9amC0Na3Er3FwwFRdILW8=";
        echo '<br/><br/>';
        echo "mediabasedpostrequest =>"; 
        $encryptText_for_order='';
        try{
            //$mediaBasedPostRequest=JWT :: encode($encryptText_for_order,$uniqueLoginKey);
            $encryptText_for_order=$this->encrypt3($OrderAPI_json,$merchant_key,'');
        }
        catch(Exception  $e)
        {
            echo $e->getMessage();
        ...
        $request["mediaBasedPostRequest"]=$encryptText_for_order;
        echo $Jrequest=json_encode($request);
        echo '<br/>';
                        
        //print_r($Jrequest); exit;
        ...
        //$URL2="https://pguat.safexpay.com/agmerchant/sdk/mediaPayments/userId/Nike119";

        #$URL2="https://www.avantgardepayments.com/agmerchant/sdk/mediaPayments/userId/DEEPAK8536";
        $URL2="https://uatportal.safexpay.com/sdk_mediaPaymentsv2_userId/AHMAD683";
        #$URL2="https://pguat.safexpay.com/agmerchant/sdk/mediaPayments/userId/AHMAD683";


        $ss = curl_init();

        curl_setopt_array($ss, array(
            CURLOPT_URL => $URL2,
            ...
            CURLOPT_POSTFIELDS =>$Jrequest,
            CURLOPT_HTTPHEADER => array(
            'userId:oC4zo4L0980WHorbhN2PFg==',
        ...
        #print_r($Jrequest);die;

        $response = curl_exec($ss);
        #print_r($response);die;

        if ($response === false ) 
        {
            echo 'cURL error: ' . curl_error($ss);exit;
        }
        else if($response=='')
        {
            echo 'Error: Payment Gatway is down.';exit;
        }

        #print_r($curl_error);exit;
        curl_close($ss);
        //print_r($response);     
        //echo "<br/>";                   #print_r($curl_error);exit;
        //print_r(base64_decode($response,$uniqueLoginKey));exit;
        // print_r($response);
        // echo "<pre>";
        // echo "<br>";
        # print_r($response);die;
         #print_r($decoded1);die;

        try{
            //$dec1=JWT :: decode($response,$uniqueLoginKey,array('HS256'));
            $dec1 = $this->decrypt3($response,$merchant_key,'');
        }
        catch(Exception  $e)
        {
            //echo $e->getMessage();
        ...
        $dec2=json_decode(json_encode($dec1),true);
        $arres=$dec2['encryptText'];
        $arress=json_decode($arres,true);

        $Lastres=$arress['paymentUrl'];

        //print_r($Lastres); exit;
        // echo "SMS Sent Successfully. Please close this window"; die;

        $this->Session->setFlash("SMS Sent Successfully");
        return $this->redirect(array('controller'=>'ExposureViews','action' => 'index_ledger2'));
...
public function paymentResponse()
{
    ...
    App::import('Vendor', 'AES');
                ...
                if(isset($_POST) && isset($_POST['txn_response']) && !empty($_POST['txn_response']))
                {
                    ...
                    $create_hash                        = encrypt($checksum, $merchant_key, 256);	
...
public function index_ledger2() 
{
    ...
    $this->layout='user';
    $data = array();
    //$exposure_date = "2022-11-30";
    $exposure_date = $this->params->query['exposure_date'];
    ...
    else
    {
        ...
        $client_det_arr = $this->RegistrationMaster->query("SELECT * FROM registration_master rm WHERE is_dd_client='1' and  `status` in ('A','D','H') ORDER BY create_date DESC"); 
...
public function add_bill() 
{
    ...
    if($result['bill_type']=='talk')
    {
        ...
        $data['AddInvParticular']['sub_category'] = 'credit';
    ...
    else if($result['bill_type']=='first_bill')
    {
        $particular_arr = array('DevelopmentCost'=>'Development Cost','SetupCost'=>'Setup Cost','Subscription'=>'Subscription');
        $i=0;
        $particulars = 'first_bill';
        foreach($particular_arr as $key=>$part)
        {
            if(!empty($result[$key]))
            {
                $rate = $result[$key];
                $data[$i]['cost_center_id'] = $cost_id;
                $data[$i]['username'] = $username;
                $data[$i]['branch_name'] = $branch_name;
                $data[$i]['cost_center'] = $cost_center;
                $data[$i]['fin_year'] = $finYear;
                $data[$i]['month_for'] = $month;
                $data[$i]['particulars'] = $part;
                $data[$i]['sub_category'] = $key;
                $data[$i]['rate'] = round($rate,2);
                $data[$i]['qty'] = 1;
                $data[$i]['amount'] = round($rate,2);
                $i++;
            } 
        }
        $this->AddInvParticular->saveAll($data);
    }
    else
    {
        ...
        $data['AddInvParticular']['fin_year'] = $finYear;
        $data['AddInvParticular']['month_for'] = $month;
        $data['AddInvParticular']['particulars'] = "For Subscription From ". date_format(date_create($start_date),"d-M-Y")." to ". date_format(date_create($end_date),"d-M-Y")."";
        $data['AddInvParticular']['sub_category'] = 'subscription';
        $data['AddInvParticular']['rate'] = round($rate,2);
        $data['AddInvParticular']['qty'] = 1;
        $data['AddInvParticular']['amount'] = round($rate,2);
    $this->AddInvParticular->save($data);
...
public function index() 
{
    ...
    $this->set('finance_yearNew', $this->BillMaster->find('list',array('fields'=>array('finance_year','finance_year'),'conditions'=>array('active'=>'1','finance_year'=>array('2022-23','2023-24','2024-25')))));
}
    ...
    if ($this->InitialInvoice->save($result))
    {
        ...
        if($category=='first_bill')
        {
           ...
           $client_id = $cost_det['CostCenterMaster']['dialdesk_client_id'];
            ...
            {
                $Transaction->rollback();
                $this->Session->setFlash(__("<h4 class=bg-active align=center style='font-size:14px'><b style='color:#FF0000'>".'Bill No. not made. Please Try Again'."</b></h4>"));
                return $this->redirect(array('controller'=>'ExposureViews','action' => 'index_ledger2'));
            }
        }
        $Transaction->commit();
...
public function get_view_approved()
{
        ...
        $result = $this->params->query;
        $branch_name = $result['branch_name'];
        $qry = "select InitialInvoice.*,cm.client from tbl_invoice InitialInvoice inner join cost_master cm on InitialInvoice.cost_center=cm.cost_center where finance_year in ('2021-22','2022-23','2023-24') and bill_no!='' and status='0'  and branch_name='$branch_name'";
        $this->set('tbl_invoice', $this->InitialInvoice->query($qry));
}
            ...
            else
            {
                ...
                if ($this->InitialInvoice->updateAll($dataN,array('id'=>$id)))
                {
                    ...
                    if($submit=='Approve')
                    {
                        ...
                        if($BillNoChange!=0)
                        {
                            $this->Session->setFlash(__("<h4 class=bg-active align=center style=font-size:14px><b style=color:#FF0000>Data Not Updated because Allready Approved</b></h4>"));
                        }
                        else
                        {
...
$role=$this->Session->read("role");
$data='';
$is_dialdee_request = false;
$is_dialdee_request  = $this->request->query['is_dialdee'];
if($is_dialdee_request=='true')
{
    $data=$this->InitialInvoice->find('first',array('conditions'=>array('id'=>$id)));
}
else if($role=='admin')
