<?php
class HomesnewController extends AppController{
        ...
        public function new_dash($today_cs_details)
        {
            // Basant code start from here
                            ...
                            if($callType ==="outbounds")
                            {
                                ...
                                SUM(if(t2.`user` !='VDAD',1,0)) `Answered`,
                                SUM(if(t2.`user` ='VDAD',1,0)) `Abandon`
                                ...
                                $this->set('Category1',$this->CallMasterOut->query("SELECT Category1,GROUP_CONCAT(if(Category2 is null,Category1,Category2),'@@',countNo)`count`,SUM(countNo) `Total` FROM (SELECT Category1,Category2,COUNT(1)`countNo` FROM call_master_out
                                where ClientId='$ClientId' $conditions AND CallType !='Upload'  GROUP BY Category1,Category2)AS tab GROUP BY Category1"));
                                
                                
                                
                                $this->set('closeloop',$this->CallMasterOut->query("SELECT Category1,Category2,COUNT(Category1) `cate1`,CloseLoopCate1,CloseLoopCate2,COUNT(CloseLoopCate1)
                                FROM call_master_out where ClientId='$ClientId' $conditions AND CallType !='Upload' GROUP BY Category1,Category2,CloseLoopCate1,CloseLoopCate2"));
                                ...
                                // echo "SELECT COUNT(Id) AS AbandCount FROM `aband_call_master` WHERE ClientId='$ClientId' $conditions AND TagStatus IS NULL"; 
                                // die;
                                ...
                                //echo $dt[0][0]['Answered']; 
                                // echo $dt[0][0]['Abandon'];
                                // echo $dt[0][0]['totaltag'];

                                $this->set('Answered',$dt[0][0]['Answered']);

                                // for mtd Doughnut

                                $dntfirstdate = date("Y-m-01");
                                if (isset($this->request->data['view_type']))
                                {
                                        ...
                                        {   
                                            ...
                                            $view_date123 =" date(t2.call_date) between '$dntfirstdate' AND '$curdate'";
                                        ...
                                        {                                    
                                            ...
                                            $view_date123 =" date(t2.call_date) between '$dntfirstdate' AND '$end'"; 
                                        ...
                                        if($view_type=="Monthly")
                                        {
                                            ...
                                            $view_date123 = " MONTH(t2.call_date) = MONTH(CURDATE()) and YEAR(t2.call_date) = YEAR(CURDATE())"; 
                                ...
                                else
                                {
                                        ...
                                        $view_date123 =" date(t2.call_date) ='$today' ";
                                }
                                    
                                
                                 $qrydnt = "SELECT COUNT(*) `Total`,
                                SUM(if(t2.`user` !='VDAD',1,0)) `Answered`,
                                SUM(if(t2.`user` ='VDAD',1,0)) `Abandon`
                                FROM asterisk.vicidial_log t2 LEFT JOIN asterisk.call_log t1 ON t1.uniqueid=t2.uniqueid left join asterisk.vicidial_agent_log t3 ON t1.uniqueid=t3.uniqueid 
                                WHERE $view_date123 $CampagnId and t2.term_reason!='AFTERHOURS' AND  t2.lead_id IS NOT NULL";
                                
                                
                                $this->vicidialLog->useDbConfig = 'db2';
                                $dtdnt= $this->vicidialLog->query($qrydnt);

                                // print_r($dtdnt);
                                // die;

                                
                                $this->set('Contacted',$dtdnt[0][0]['Answered']);
                                $this->set('NotContacted',$dtdnt[0][0]['Abandon']);
                                ...
                                $this->vicidialLog->useDbConfig = 'db2';
                                $graph_date= $this->vicidialLog->query($qry1);
                                $this->set('graph_date_wise',$graph_date);

                                // 2 end graph date wise 

                                

                                // 3 start Ticket case weekwise

                                    // Ticket Case Analysis
                            

                                    $qry1 = "SELECT Category1,DAY(calldate) dayt,date_format(calldate,'%b-%y') week_month_year FROM call_master_out cm WHERE
                                    $weeks_mtd_ticket_conditions AND clientId='$ClientId' 
                                    order by date(calldate)"; 

                                    $week_data= $this->CallMasterOut->query($qry1);
                                    $week_arr = array();
                                    $category_arr = array();
                                    $week_divider = array();
                                    $wm_array = array();
                                 ...
                                 // 3 end  Ticket case weekwise

                                 // 4 start ticket case today,mtd

                                    // for ticket case today,mtd

                                    $tctm_today_qry="SELECT SUM(IF(CloseLoopCate1 = 'Done',1,0)) AS TClose , SUM(IF(CloseLoopCate1 = 'Pending',1,0)) AS TOpen FROM `call_master_out`
                                    WHERE  DATE(CloseLoopingDate) = CURDATE() AND ClientId='$ClientId'";
                                    
                                    $tctm_today_row = $this->CallMasterOut->query($tctm_today_qry);
                                    ...
                                    $tctm_mtd_qry="SELECT SUM(IF(CloseLoopCate1 = 'Done',1,0)) AS TClose , SUM(IF(CloseLoopCate1 = 'Pending',1,0)) AS TOpen FROM `call_master_out`
                                    WHERE DATE(CloseLoopingDate) BETWEEN ('$first_day_this_month') AND ('$current_date') AND ClientId='$ClientId' ";
                                    
                                    $tctm_mtd_row = $this->CallMasterOut->query($tctm_mtd_qry);

                                    $this->set('tctm_today_row',$tctm_today_row);
                                    $this->set('tctm_mtd_row',$tctm_mtd_row);


                                 // 4 end ticket case today,mtd

                                 // 5&6 start open & close case status
                                ...
                                // 5&6 end open & close case status

                                // table start ticket by source
                            ...
                            else 
                            {
                                ...
                                // 1 end graph
                                 ...
                                 // 5 start open & close case status
