...
class ObsrDetailsController extends AppController{
    ...
    public $uses=array('CampaignName','ObecrMaster','ObField','ObFieldValue','ObcallMasterout','CallMasterOut','ObCatMaster','ObField','ObAllocationMaster','CampaignName','ObCloseFieldData','ObCloseFieldDataValue','LogincreationMaster');
	...
	$clientId = $this->Session->read('companyid');
        ...
        if($this->Session->read("role") =="agent"){
            ...
            $Campaign =$this->CampaignName->find('list',array('fields'=>array("id","CampaignName"),'conditions'=>array('ClientId'=>$clientId,'CampaignStatus'=>'A','id'=>$outboundAccess)));
        }
        else{
        ...
        */
        ...
        if($this->request->is("POST")){
           ...
           // print_r($AllocationId);die;
            ...
            if(isset($search['Category3']) && $search['Category3'] !=""){$data['CallMasterOut.Category3']=$search['Category3'];}else{unset($data['CallMasterOut.Category3']);}
            if(isset($search['Category4']) && $search['Category4'] !=""){$data['CallMasterOut.Category4']=$search['Category4'];}else{unset($data['CallMasterOut.Category4']);}
            if(isset($search['Category5']) && $search['Category5'] !=""){$data['CallMasterOut.Category5']=$search['Category5'];}else{unset($data['CallMasterOut.Category5']);}
            ...
            //print_r($campaign_field);die;
            $allocation_field = $this->get_fields($total_count,"obcd.Field");

            $header_field=array_merge(array('LeadId','Id','Out Call Id','Call From'),$category_field1,$capcture_field,array('CallDate','Call Action','Call Sub Action','Call Action Remarks','Closer Date','Follow Up Date','Call Created','Reschedule Data'),$close_capcture_field,$campaign_field);
            $values_field=array_merge(array('LeadId','Id','SrNo','MSISDN'),$category_field,$callmaster_field,array('CallDate','CloseLoopCate1','CloseLoopCate2','closelooping_remarks','CloseLoopingDate','FollowupDate','callcreated','RescheduleData'),$close_callmaster_field,$allocation_field);

            // 3 months data
            $threeMonthsAgo = date('Y-m-d', strtotime('-3 months'));
            if (!isset($search['startdate']) || strtotime($search['startdate']) < strtotime($threeMonthsAgo)) {
                $data['date(CallMasterOut.CallDate) >='] = $threeMonthsAgo;
            }
    ...
    public function get_subtype1(){
        $this->layout='ajax';
        if($_REQUEST['campid']){
            $clientId=$this->Session->read('companyid');
            $this->set('subtype1',$this->ObecrMaster->find('list',array('fields'=>array('ecrName','ecrName'),'conditions'=>array('CampaignId'=>$_REQUEST['campid'],'Client'=>$clientId,'Label'=>3),'group'=>'ecrName','order'=>array('ecrName'=>'asc'))));	    
        }
    }

    public function get_subtype2(){
        $this->layout='ajax';
        if($_REQUEST['campid']){
            $clientId=$this->Session->read('companyid');
            $this->set('subtype2',$this->ObecrMaster->find('list',array('fields'=>array('ecrName','ecrName'),'conditions'=>array('CampaignId'=>$_REQUEST['campid'],'Client'=>$clientId,'Label'=>4),'group'=>'ecrName','order'=>array('ecrName'=>'asc'))));	    
        }
    }
