...
class AgentsController extends AppController{
	...
	public $uses=array('FieldMaster','FieldValue','EcrMaster','CallMaster','ClientMaster','ObField','ObFieldValue','ObEcr','CampaignName','ObCampaignDataMaster','ObAllocationMaster','UploadExistingBase','CloseLoopMaster','TimePeriod','SocialMediaFeedback','MailMaster','CallFlow');
	...
	public function beforeFilter(){
		parent::beforeFilter();
		$this->Auth->allow('save_tagging','campaignName','index','getECRName','search_result','getChild','integerToRoman','dlfaddress','getResolution');
	...
	public function index() {
		...
		$history = $this->CallMaster->find('all',array('conditions'=>array('MSISDN'=>$this->Session->read("pno"),'ClientId'=>$clientId),'order'=>array('CallDate'=>'desc')));
                ...
                $this->set('fieldSearch',$fieldSearch);
        ...
        public function save_tagging(){
            if($this->request->is("POST")){
                ...
                {
                    $fName = $field['FieldMaster']['fieldNumber'];
                    if($field['FieldMaster']['FieldValidation']=='Datepicker')
                    {
                        $data['CallMaster']['Field'.$fName] = date("Y-m-d",strtotime($data['CallMaster']['Field'.$fName]));
                    }
                }
    ...
    public function getResolution(){
        $this->layout ='ajax';
            if($this->request->is("POST")){
               ...
               $res_qry = "SELECT resolution  FROM call_flow WHERE 
                ...
                CONCAT(',',subtype1),''),
                IF(`subtype2` IS NOT NULL OR `subtype2` !='',CONCAT(',',subtype2),''))='".implode(",",$data_check)."' and client_id='$client_id' $wheretag limit 1";

                $Res_data = $this->CallFlow->query($res_qry);

                if(empty($Res_data))
                {
                    $res_qry2 = "SELECT resolution  FROM call_flow WHERE 
                    CONCAT(category, IF(`type` IS NOT NULL OR `type` !='',CONCAT(',',`type`),''),
                    IF(`subtype` IS NOT NULL OR `subtype` !='',CONCAT(',',subtype),''), IF(`subtype1` IS NOT NULL OR `subtype1` !='',
                    CONCAT(',',subtype1),''),
                    IF(`subtype2` IS NOT NULL OR `subtype2` !='',CONCAT(',',subtype2),''))='' and client_id='$client_id' $wheretag limit 1";
                    $Res_data = $this->CallFlow->query($res_qry2);
                }
                echo ("{$Res_data[0]['call_flow']['resolution']}");
