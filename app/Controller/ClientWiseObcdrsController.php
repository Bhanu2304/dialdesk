<?php
class ClientWiseObcdrsController extends AppController
{
...
and $campaignId and t2.term_reason!='AFTERHOURS'";
    ...
    public function cdr_mis(){
        if($this->request->is("POST")){
            ...
            header("Content-Disposition: attachment; filename=client_wise_ob_cdr_report.xls");
            ...
            //$campaignId ="t2.campaign_id in('dialdesk')";
            $campaignId ="t2.campaign_id in(".$GroupId.")";
            ...
            //$this->vicidialLog->useDbConfig = 'db2';
            $Data=$this->vicidialLog->query($qry);
            
            $TagArr = $this->CallRecord->query("SELECT LeadId FROM `call_master_out` WHERE DATE(CallDate) BETWEEN '$FromDate' AND '$ToDate' AND ClientId='$clientId'");
            
            $TagData=array();
            foreach($TagArr as $row){
                $TagData['LEADID'][]= $row['call_master_out']['LeadId'];
            }
            ...
            <table cellspacing="0" border="1">
                ...
                <?php if (in_array($dt['t2']['lead_id'], $TagData['LEADID'])){?>
                <tr>
                    <td><?php echo $i++;?></td>
