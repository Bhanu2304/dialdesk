...
class Homes2Controller extends AppController{
    ...
    public function new_dash($today_cs_details)
    {
        // Basant code start from here
                        ...
                        //      $tctm_today_qry="SELECT SUM(IF(CloseLoopStatus = 'CLOSE',1,0)) AS TClose , SUM(IF(CloseLoopStatus = 'OPEN',1,0)) AS TOpen FROM `call_master`
                        //    WHERE  DATE(CloseLoopingDate) = CURDATE() AND ClientId='$ClientId'";
                        ...
                        //     $tctm_mtd_qry="SELECT SUM(IF(CloseLoopStatus = 'CLOSE',1,0)) AS TClose , SUM(IF(CloseLoopStatus = 'OPEN',1,0)) AS TOpen FROM `call_master`
                        //    WHERE DATE(CloseLoopingDate) BETWEEN ('$first_day_this_month') AND ('$current_date') AND ClientId='$ClientId' ";
                            
                        //    $tctm_mtd_row = $this->CallMaster->query($tctm_mtd_qry);
                            // print_r($tctm_today_row);die;
...

                    {
                        ...
                        // for Plan Master
                            ...
                            $planmaster_arr = $this->PlanMaster->query("SELECT PlanName,PeriodType,CreditValue,InboundCallCharge,EmailCharge,SMSCharge,OutboundCallCharge,InboundCallChargeNight,InboundCallCharge,RentalAmount FROM plan_master WHERE `Id`=$planId");
...
public function index_ledger2($company_id,$get_from_date,$get_to_date,$view_type2) 
{
    ...
    if($view_type2=="Custom"){
        $from_date = date('Y-m-d', strtotime($this->request->data['fdate2']));
        $to_date = date('Y-m-d', strtotime($this->request->data['ldate2']));
        $cs_bal =0;
    }
            ...
            foreach($cost_cat_arr as $cost)
            {
                ...
                if(strtotime($from_date)>strtotime('2022-04-01'))
                {
                    ...
                    while($date_capture_start<=strtotime($day_before))
                    {
                        ...
                        $consume_credit2 = $this->get_consumption2($clientId,$cap_date,$consume_credit,'new',$start_date);
...
public function get_billed($cost_center,$inv_date,$rental_credit,$balance_credit,$op2_ledger,$op2_credit)
{
    ...
    public function get_consumption2($clientId,$cap_date,$op2_value,$type,$start_date)
    {
        //getting consumption from table 
        $select_consumption = "SELECT sum(cm_total) total FROM billing_consume_daily  WHERE client_id='$clientId' and date(cm_date)>='$start_date' and date(cm_date)='$cap_date'";
        $rsc_consuption_arr = $this->RegistrationMaster->query("$select_consumption");
        //print_r($rsc_consuption_arr);exit;
        $op2_value = 0;
        foreach($rsc_consuption_arr as $consume)
