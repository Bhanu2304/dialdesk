...
class AppController extends Controller {
    ...
    var $uses = array('LogincreationMaster','PagesMaster','Admin','UserType','PagesMaster1');
        function Controler(){
    ...
    function getLatestData(){
        ...
        if($this->Session->read("role") ==="admin"){
            $page = $this->PagesMaster1->find('all',array('order'=>"FIELD(PagesMaster1.parent_id,PagesMaster1.priority) ASC"));
            foreach($page as $row){
    ...
    function buildLeftMenu($parent, $menu) { 
            ...
            if($this->Session->read("role") ==="admin"){
                $user  = $this->Admin->find('first',array('fields'=>array('user_right','user_type'),'conditions' =>array('id' =>$this->Session->read("admin_id"))));
                
                $userType = $user['Admin']['user_type'];
                $user_acc  = $this->UserType->find('first',array('fields'=>array('access','parent_access'),'conditions' =>array('user_type' =>$userType)));
                $parent_right = explode(",",$user_acc['UserType']['parent_access']);
                $child_right = explode(",",$user_acc['UserType']['access']);
