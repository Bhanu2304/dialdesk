...
class AccesController extends AppController {
    ...
    public function index() {
        ...
        $menus = $this->PagesMaster1->query("SELECT * FROM pages_master1 where active='1' ORDER BY parent_id, priority"); 
        ...
        $sel = "SELECT id,username FROM tbl_user WHERE  user_active='1'  ORDER BY username ";
    ...
    public function save() {
        ...
        $rides = $this->PagesMaster1->query("Update tbl_user set access='" . $child . "',parent_access='" . $pp . "' WHERE id='" . $username . "'");
    ...
    public function get_access_id() {
        ...
        $rides = $this->PagesMaster1->query("SELECT id, username,access,parent_access FROM tbl_user WHERE id='" . $user_id . "'");
         echo json_encode($rides); exit;
//         
//         $user_id = $this->params['url']['user'];
//        $user=$this->Admin->find('first',array('conditions' =>"id='$user_id'"));
//        //print_r($user);exit;
//       $userAss = $user['Admin']['user_right']; 
//        
//                
//        //$rides = $this->PagesMaster1->query("SELECT id,user_type username,access,parent_access FROM tbl_user WHERE id in ('" . $userAss . "')");
//         echo json_encode(array('0'=>array('user_type'=>array('access'=>$userAss,'parent_access'=>$userAss)))); exit;
    }    
