...
class MisAndReportMatrixsController extends AppController{
	...
	public $uses=array('ReportMatrixMaster','MisReportMatrixMaster','ReportMaster','CampaignName','ObReportMatrixMaster');
	
	public function beforeFilter(){
		parent::beforeFilter();
		$this->Auth->allow(
                        'ob_matrix',
                        'save_obreport_matrix',
                        'delete_obreport_matrix',
			'save_report_matrix',
			'view_report_matrix',
	...
	public function save_report_matrix(){
		...
		if(isset($this->request->query['id'])){
			$id=$this->request->query['id'];
			$clientid=$this->Session->read('companyid');
			$result = $this->ObReportMatrixMaster->find('first',array('conditions' =>array('id' =>$id,'client_id' =>$clientid)));	
			$this->set('matrixArr',$result);
		}
                               
                $ClientId = $this->Session->read('companyid');
                $campaign_list =$this->CampaignName->find('list',array('fields'=>array("id","CampaignName"),'conditions'=>array('ClientId'=>$ClientId,'CampaignStatus'=>'A')));
                //print_r($campaign_list);exit;
                $this->set('campaign_list',$campaign_list);
               $result1 = $this->ObReportMatrixMaster->find('all',array('conditions' =>array('client_id' => $this->Session->read('companyid'))));
               
		$this->set('data',$result1);
                
	}
        
        public function save_obreport_matrix(){
		if($this->request->is('Post')){
			if($this->request->data['daywise'] !=""){
				$report_val=$this->request->data['daywise'];
			}
			...
			if($_POST['submit'] && $_POST['submit']=='Update'){
				...
				$this->ObReportMatrixMaster->updateAll($data,array('id'=>$this->request->data['updateid'],'client_id' => $this->Session->read('companyid')));
			}
			else{
				$this->ObReportMatrixMaster->save($data);
			}
			$this->redirect(array('controller' => 'MisAndReportMatrixs','action'=>'ob_matrix'));
		}
	}
	...
	public function delete_report_matrix(){
        ...
        public function delete_obreport_matrix(){
		$id  = $this->request->query['id'];
		$this->ObReportMatrixMaster->delete(array('id'=>$id,'client_id' => $this->Session->read('companyid')));
		$this->redirect(array('action' => 'ob_matrix'));
	}
}
?>