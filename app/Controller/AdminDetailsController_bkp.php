...
class AdminDetailsController extends AppController{
    ...
    public function beforeFilter() {
        parent::beforeFilter();
            $this->Auth->allow('update_did','add_did','delete_did','clientdid','view_client','client_details','get_city','update_client','client_permission','update_client_permissions','view_client_request','update_request','get_client_id');
	...
	$result = $this->ClientRequestMaster->find('all',
        ...
        public function update_request(){
            if($_REQUEST['id']){
               ...
               if($reqtype ==="email"){
                    $dataArr=array('email'=>"'".$reqdata."'",'modify_date'=>"'".date('Y-m-d H:i:s')."'",);
               }
               else if($reqtype ==="phone"){
                    $dataArr=array('phone_no'=>"'".$reqdata."'",'modify_date'=>"'".date('Y-m-d H:i:s')."'",);
               }
        ...
        public function add_did(){
            if($this->request->is('Post')){
                ...
                if (strlen($data['did_number']) < 8){
                    $this->Session->setFlash('<span style="color:red;">Did number should required 8 digit.</span>');
                }
                else if (strlen($data['customer_care_number']) < 8){ 
                    $this->Session->setFlash('<span style="color:red;">Customer care number should required 8 digit.</span>');
                }
