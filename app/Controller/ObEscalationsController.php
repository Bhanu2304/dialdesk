<?php
class ObEscalationsController extends AppController{
    
    public $helpers = array('Html', 'Form','Js');
    public $components = array('RequestHandler');
    public $uses=array('ObEscalation','CampaignName','ObclientCategory','LogincreationMaster','ObfieldCreation',
        'Crone','RegistrationMaster','ObMatrix','ObSMSText','TimePeriod','ObTemplate','ObAgentWhatsapp');
	
    public function beforeFilter() {
        parent::beforeFilter();
        if($this->Session->check('companyid'))
        {
            $this->Auth->allow('index','add','view','view_value','delete','getParent','view_fields','view_fields2','matrix','get_campaign_ecr',
                    'get_campaign_req_fields','getEcr','smsText','smsTextCustomer','time_period','save_alert_esclation',
                    'delete_matrix','save_customer_smstext','save_customer_whatsapptext','delete_sms','save_smstext','view_edit_data','update_alert_esclation',
                    'get_edit_ecr','update_customer_smstext','save_agent_alert','delete_whatsapp_caller');
	}
	else
	{
            $this->Auth->deny('index','add','view','view_value','delete','getParent','view_fields','matrix','getEcr','smsText','smsTextCustomer','time_period','save_alert_esclation','delete_matrix','save_customer_smstext','save_customer_whatsapptext','delete_sms','save_smstext','view_edit_data','update_alert_esclation','get_edit_ecr','update_customer_smstext','save_agent_alert','delete_whatsapp_caller');
	}
    }
    ...
    //$Category =$this->ObclientCategory->find('list',array('fields'=>array("id","name"),'conditions'=>array('Client'=>$ClientId,'Label'=>'1')));
    //$Category['All'] = 'All';
    ...
    $Campaign['All'] = 'All';
    $this->set('Category',$Campaign);
						
    $data=$this->ObclientCategory->find('all',array('fields'=>array('id','Label','ecrName','id','parent_id'),'conditions'=>array('Client'=>$ClientId)));
    $Ecr = $this->ObclientCategory->find('first',array('fields'=>array('Ecr'),'conditions'=>array('Client'=>$ClientId)));
    $this->set('data',$data);
    $field = $this->ObfieldCreation->find('all',array('fields'=>array('id','FieldName'),'conditions'=>array('ClientId'=>$ClientId)));
		
    $fields_send = array();
    foreach($field as $f):
        $fields_send[$f['ObfieldCreation']['FieldName']] = $f['ObfieldCreation']['FieldName'];
    endforeach;
    ...
    for($i =1; $i<$Ecr['ObclientCategory']['Ecr']; $i++)
    {
    ...
    if(isset($this->request->query['id']))
    {
		...
		//$this->set('data',$this->ObclientCategory->find('all',array('fields'=>array('id','Label','ecrName','id','parent_id','Escalation.username'),'joins'=>array(array('table'=>'escalation_master','alias'=>'Escalation','type'=>'Left','conditions'=>array('Escalation.ecrId=ObclientCategory.id'))),'conditions'=>array('Client'=>$ClientId))));
    ...
    $this->set('data',$this->ObclientCategory->find('all',array('fields'=>array('id','Label','ecrName','id','parent_id'),'conditions'=>array('Client'=>$ClientId))));
    ...
    public function view_fields(){
        ...
        $this->set('data1',$this->ObMatrix->find('all',array('conditions'=>array('alertType'=>'Alert','clientId'=>$ClientId))));
        $this->set('data2',$this->ObMatrix->find('all',array('conditions'=>array('alertType !='=>'Alert','clientId'=>$ClientId))));
        $this->set('data3',$this->ObSMSText->find('all',array('conditions'=>array('clientId'=>$ClientId,'sendType'=>'0'))));
        $this->set('data4',$this->ObSMSText->find('all',array('conditions'=>array('clientId'=>$ClientId,'sendType'=>'1')))); 
        $this->set('data5',$this->ObTemplate->find('all',array('conditions'=>array('clientId'=>$ClientId)))); 

        // View Virtual Fields
        
        $field = $this->ObfieldCreation->find('all',array('fields'=>array('id','FieldName'),'conditions'=>array('ClientId'=>$ClientId,'FieldStatus'=>NULL)));
        $Ecr = $this->ObclientCategory->query("SELECT MAX(label) `label` FROM ecr_master WHERE `client` = '$ClientId' limit 1");
        //print_r($Ecr); exit;
    $fields_send = array();
    ...
    public function get_campaign_ecr()
    {
        $ClientId = $this->Session->read('companyid');
        $campaign_det = $_REQUEST['campaign_id'];
        $Arr = explode('@@',$campaign_det);
        $campaign_id = $Arr['0'];
        $Ecr = $this->ObclientCategory->query("SELECT MAX(label) `label` FROM obecr_master WHERE `client` = '$ClientId' and campaignid='$campaign_id' limit 1");
        
        echo '<optgroup label="Select Scenario">
</optgroup>'; 
        ...
        {
            if($i ==1){
            echo '<option value="'.'Category'.$i.'">'.'Scenario'.'</option>';
            }
            else{
                $j=$i-1;
              echo '<option value="'.'Category'.$i.'">'. 'Sub Scenario '.$j.'</option>';  
            }

        }
		exit;
    //$field_send2['Select Scenario'] = $fields_send; 
    ...
    public function get_campaign_req_fields()
    {
        $ClientId = $this->Session->read('companyid');
        $campaign_det = $_REQUEST['campaign_id'];
        $Arr = explode('@@',$campaign_det);
        $campaign_id = $Arr['0'];
        $field = $this->ObfieldCreation->find('all',array('fields'=>array('id','FieldName'),'conditions'=>array('ClientId'=>$ClientId,'campaignid'=>$campaign_id,'FieldStatus'=>NULL)));
        
        echo '<optgroup label="Select Fields">
</optgroup>'; 
        
        
        foreach($field as $f):
            echo '<option value="'.$f['ObfieldCreation']['FieldName'].'">'.$f['ObfieldCreation']['FieldName'].'</option>';;
        endforeach;
    exit;
     ...
     public function view_edit_data(){
        ...
        if(isset($_REQUEST['id'])){
            if($_REQUEST['type'] ==="tab1"){ 
               $data1=$this->ObMatrix->find('first',array('conditions'=>array('id'=>$_REQUEST['id'],'clientId'=>$ClientId)));
               $this->set('data1',$data1['ObMatrix']);
            }
            ...
            if($_REQUEST['type'] ==="tab3"){ 
               $data2=$this->ObSMSText->find('first',array('conditions'=>array('id'=>$_REQUEST['id'],'clientId'=>$ClientId,'sendType'=>'0')));
               $this->set('data2',$data2['ObSMSText']);
            } 
            if($_REQUEST['type'] ==="tab4"){ 
               $data3=$this->ObSMSText->find('first',array('conditions'=>array('id'=>$_REQUEST['id'],'clientId'=>$ClientId,'sendType'=>'1')));
                $this->set('data3',$data3['ObSMSText']);
            }
    ...
    public function save_alert_esclation(){
        if($this->request->is('POST')){
            $ClientId = $this->Session->read('companyid');
            $data = $this->request->data['ObEscalations'];
            ...
            foreach($data as $k=>$v){
                ...
                $keys = array('campaignid','category','type','subtype','subtype1','subtype2');
                if(in_array($k,$keys)){
                    $Arr = explode('@@',$v);
                    $data1[$k] = $Arr[0];
                    $data1[$k.'Name'] = $Arr[1];
            ...
            $this->ObMatrix->save($data1);
            if($data['tabType'] ==="tab1"){
                $this->Session->setFlash("Alearts Define  SuccessFully.");
            }
            if($data['tabType'] ==="tab2"){
                $this->Session->setFlash("Esclation ObMatrix Define  SuccessFully.");
            }
            return $this->redirect(array('controller'=>'ObEscalations','action'=>'view_fields','?'=>array('tab'=>$data['tabType'])));
        }  
    }
    ...
    public function update_alert_esclation(){
        if($this->request->is('POST')){
			...
			if(isset($data1['campaignid']) && $data1['campaignid'] !=""){$dataArr['campaignid']="'".$data1['campaignid']."'";}else{unset($dataArr['campaignid']);}
            if(isset($data1['campaignidName']) && $data1['campaignidName'] !=""){$dataArr['campaignidName']="'".$data1['campaignidName']."'";}else{unset($dataArr['campaignidName']);}
           ...
           // print_r($dataArr);die;
            ...
            if(!empty($dataArr)){
                $this->ObMatrix->updateAll($dataArr,array('id'=>$data['id'],'clientId'=>$ClientId));
            }
    ...
    public function update_customer_smstext(){
        if($this->request->is('POST')){
            ...
            if(!empty($dataArr)){
                $this->ObSMSText->updateAll($dataArr,array('id'=>$data['id'],'clientId'=>$ClientId));
            }
    ...
    public function save_customer_smstext(){
        if($this->request->is('POST')){
            ...
            $this->ObSMSText->save($data1);
            $this->Session->setFlash("SMS Text Define  SuccessFully.");
            return $this->redirect(array('controller'=>'ObEscalations','action'=>'view_fields','?'=>array('tab'=>$data['tabType'])));
        }
    }

    public function save_customer_whatsapptext(){
        if($this->request->is('POST')){
            $ClientId = $this->Session->read('companyid');
            $data = $this->request->data['ObEscalations'];
            $data['clientId']=$ClientId;
            ...
            $this->ObTemplate->save($data1);
            $this->Session->setFlash("Whatsapp Text Define  SuccessFully.");
            return $this->redirect(array('controller'=>'ObEscalations','action'=>'view_fields','?'=>array('tab'=>$data['tabType'])));
        }
    }
    ...
    public function delete_matrix(){
        ...
        $this->ObMatrix->deleteAll(array('id'=>$id,'clientId' => $this->Session->read('companyid')));
        $this->redirect(array('controller'=>'ObEscalations','action'=>'view_fields','?'=>array('tab'=>$_REQUEST['tab'])));
    }
    
    public function delete_sms(){
        $id=$this->request->query['id'];
        $this->ObSMSText->deleteAll(array('id'=>$id,'clientId' => $this->Session->read('companyid')));
        $this->redirect(array('controller'=>'ObEscalations','action'=>'view_fields','?'=>array('tab'=>$_REQUEST['tab'])));
    }

    public function delete_whatsapp_caller()
    {
        $id=$this->request->query['id'];
        $this->ObTemplate->deleteAll(array('id'=>$id,'clientId' => $this->Session->read('companyid')));
        $this->redirect(array('controller'=>'ObEscalations','action'=>'view_fields','?'=>array('tab'=>$_REQUEST['tab'])));
    }
    ...
    if($this->request->is('POST'))
    {
        $data = $this->request->data['ObEscalation'];
    ...
    $data = $this->ObMatrix->find('all',array('conditions'=>array('clientId'=>$ClientId)));
    $category = $this->ObclientCategory->find('list',array('fields'=>array("id","EcrName"),'conditions'=>array('label'=>'1')));
...
public function smsText()
{
    ...
    if($this->request->is('POST'))
    {
        ...
        $data1['createby'] = $ClientId;
         ...
         $this->Session->setFlash("Added SuccessFully");
         return $this->redirect(array('controller'=>'ObEscalations','action'=>'smsText'));
       
    }
    $clientData = $this->RegistrationMaster->find('list',array('fields'=>array('company_id','company_name'),'conditions'=>array('status'=>'A')));
    $category = $this->ObclientCategory->find('list',array('fields'=>array("id","name"),'conditions'=>array('label'=>'1')));
...
public function getParent()
{
    ...
    $data = $this->ObclientCategory->find('list',array('fields'=>array("id","name"),'conditions'=>$data));
    $this->set('data',$data);
...
public function add() 
{
	...
	if(empty($data['field'])){ return;}
		...
		if($ids = $this->ObfieldCreation->find('first',array('fields'=>array('Priority'),'conditions'=>array('ClientId'=>$ClientId,'FieldName'=>$str))))
                {
                    $arrCapture[$i++] = $ids['ObfieldCreation']['Priority'];                                                    
		}
                $strArr[] = $str;
            }
...
public function save_agent_alert(){
    if($this->request->is('POST')){
        $ClientId = $this->Session->read('companyid');
        $data = $this->request->data['ObEscalations'];
        $data['clientId']=$ClientId;
        ...
        $data1['createdate'] = date('Y-m-d H:i:s');
        $data1['created_by'] = $ClientId;

        $this->ObAgentWhatsapp->save($data1);
       
        $this->Session->setFlash("Alerts Define SuccessFully.");
