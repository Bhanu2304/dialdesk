...
function validateExport(url){
    ...
    if(fdate ===""){
        ...
        if(url ==="download"){
            $('#validate-form').attr('action','<?php echo $this->webroot;?>OutboundReports/agent_apr_new_excel');
        }
        if(url ==="view"){
            $('#validate-form').attr('action','<?php echo $this->webroot;?>OutboundReports/agent_apr_new');
        }
