...
if(isset($campaignid['RegistrationMaster']['multilang_ivrs']) && $campaignid['RegistrationMaster']['multilang_ivrs'] !=""){ 
    $exp=  explode(",", $campaignid['RegistrationMaster']['multilang_ivrs']);
    $MultiLangName=array();
    for($i=0;$i<count($exp);$i++){
        $MultiLangName[]=str_replace("'", '', $exp[$i]);
    } 
