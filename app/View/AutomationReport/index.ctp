...
function validateExport(url){
    ...
    return false;
        ...
        if(url === "download"){
            $('#validate-form').attr('action','<?php echo $this->webroot;?>AutomationReport/export_corrective_report');
        }
        if(url === "view"){
            $('#validate-form').attr('action','<?php echo $this->webroot;?>AutomationReport/index');
        }
    ...
    <div data-widget-group="group1">
        <div class="panel panel-default" data-widget='{"draggable": "false"}'>
            ...
            <div class="panel-body">
                ...
                <?php echo $this->Form->create('AutomationReport',array('action'=>'view','id'=>'validate-form','class'=>'form-horizontal row-border','data-parsley-validate')); ?>
            ...
            <div class="panel-body no-padding scrolling">
                <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered">
                    ...
                    <thead>
                        <tr>
                            <th></th>
                        <?php foreach($data as $dataLabel=>$dataSub) { echo "<th>$dataLabel</th>"; } ?>
                        </tr>
                       
                    </thead>
                    <tbody>
                    <?php foreach($datetimeArray as $dateLabel){ ?>
                        <tr>
                          <th><?php echo $dateLabel; ?></th>
                        </tr>
                    <tr>
                        <td>Open</td>
                        <?php $total_open=0; foreach($data as $dataLabel=>$dataSub) { ?>
                        
                            <!-- <td><?php //print_r($dataSub); ?></td> -->
                            <?php   echo "<td>{$dataSub['open'][$dateLabel]}</td>";   ?>
                        	
                        <?php $total_open+=$dataSub['open'][$dateLabel];} ?>	
                    </tr>
                    <tr>
                        <td>Close</td>
                        <?php $total_close=0; foreach($data as $dataLabel=>$dataSub) { ?>
                        
                            <!-- <td><?php //print_r($dataSub); ?></td> -->
                            <?php echo "<td>{$dataSub['close'][$dateLabel]}</td>"; ?>
                            <?php $total_close+=$dataSub['close'][$dateLabel]; ?>
