...
<script>
    function inbound_getChild(id,clientId)
{
...
for(; i<5; i++)
{
	...
	$.post("<?php echo $this->webroot?>Agents/getChild",
        {
          Parent: parent,
          Label: label,
          clientId: clientId
        ...
        for(var sc=0; sc<scen_list.length; sc++)
        {
            
            params["scenario"+sc2] = scen_list[sc].value;
            sc2++;
        }
        params['client_id'] = clientId;
        params['language'] = language;
        // console.log(params);
        $.post("<?php echo $this->webroot?>Agents/getResolution",
        params,

        function(data){
            if(data!='')
			{
                document.getElementById("resolution_text").innerHTML = data;
            }
        });
...
<div id="tabs-2">
 <div class= "col-md-9">
                    ...
                    <div class="form-group">
                        ...
                        <?php
                            ...
                            for($i =0; $i <1; $i++)
                            {
                                ...
                                echo "<div class=\"col-sm-2\">".$this->Form->input('Category'.$key, array('label'=>false,'options'=>$options,'required'=>true,'empty'=>'Select Scenarios','class'=>'form-control scen','onChange'=>'inbound_getChild(this.id,"'.$clientId.'")')).
    ...
    <div class="col-md-3"  style="overflow:scroll;height:585px;text-align:justify;font-size:12px;">
        <select id='lang' name='lang' style='float: right;'>
                        <option value='En'>En</option>
                        <option value='Hi'>Hi</option>
                    </select><br>
        <div id ="resolution_text"> </div>            
            <?php //echo $this->Form->textArea('resolution',array('label'=>false,'class'=>'form-control','id'=>'resolution_text','rows'=>'15','cols'=>'70','readonly'));?>
        </div>
 	...
 	<?php
		...
		foreach($history as $his){
			...
			foreach($keys as $k){ 
                        ...
                        // }
                        foreach($fieldSearch as $header)
                        {  
                            // print_r($header);
                            if($header['FieldMaster']['FieldValidation']=='Datepicker' && !empty($his['CallMaster']['Field'.$header['FieldMaster']['fieldNumber']]))
                            {

                              echo "<td>".date("d-m-Y",strtotime($his['CallMaster']['Field'.$header['FieldMaster']['fieldNumber']]))."</td>";

                            }else{

                              echo "<td>".$his['CallMaster']['Field'.$header['FieldMaster']['fieldNumber']]."</td>";
			            ...
			            echo "<td>".date("d-m-Y",strtotime($his['CallMaster']['CallDate']))."</td>";
                    echo "</tr>";
                }
	?>
      </tbody>
