    ...
    <div data-widget-group="group1">
        <div class="panel panel-default" data-widget='{"draggable": "false"}'>
            ...
            <div class="panel-body">
                <?php echo $this->Form->create('AdminUsers',array('action'=>'save_login','id'=>'save_login_creation')); ?>
                    <div class="col-md-12">
                        <div class="col-xs-12">
                        ...
                        <div class="row">
                            <div class="col-sm-6 col-sm-offset-8">
                                <div class="btn-toolbar">
                                    <input type="button" onclick="validateAdminCreation('<?php echo $this->webroot;?>AdminUsers/check_exist_phone','<?php echo $this->webroot;?>AdminUsers/checkexistmail')" class="btn btn-web" value="Submit" >
                    ...
                    <input type="hidden" id="eurl" value="<?php echo $this->webroot;?>AdminUsers/checkSmtpValidation"  />
                    <input type="hidden" id="eeurl" value="<?php echo $this->webroot;?>AdminUsers/check_exist_email" />
                    <input type="hidden" id="purl" value="<?php echo $this->webroot;?>AdminUsers/sendotp" />
                    <input type="hidden" id="loginurl" value="<?php echo $this->webroot;?>AdminUsers" />
                    <input type="hidden" id="saveLogin" value="<?php echo $this->webroot;?>AdminUsers/save_login_creation" />
                <?php $this->Form->end(); ?>
            </div>
        </div> 
                        ...
                        <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered datatables" >
                            ...
                            <tbody>
                                <?php $i = 1;foreach($data as $row){?>
                                    <tr>
                                        <td><?php echo $i++;?></td>
                                        <td><?php echo $row['Admin']['name'];?></td>
                                        <td><?php echo $row['Admin']['phone'];?></td>
                                        <td><?php echo $row['Admin']['Email'];?></td>
                                        <td><?php echo $row['Admin']['designation'];?></td>
                                        <td><?php if($row['Admin']['user_active']=='1') { echo 'Active';} else { echo 'De-Active';} ?></td>
                                        <td>
                                            <a  href="#" data-toggle="modal" data-target="#loginUpdate" onclick="view_edit_admin('<?php echo $row['Admin']['id'];?>')" >
                                                <label class="btn btn-xs btn-midnightblue btn-raised">
                                                    <i class="fa fa-edit"></i><div class="ripple-container"></div>
                                                </label>
                                            </a> 
                                            <a href="#" onclick="deleteData('<?php echo $this->webroot;?>AdminUsers/delete_user?id=<?php echo $row['Admin']['id'];?>')" >
...
<div class="modal fade" id="loginUpdate" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
           ...
           <?php echo $this->Form->create('AdminUsers',array('action'=>'update_login')); ?>
            <div class="modal-body">
            ...
            <div class="modal-footer">
               ...
               <input type="button" onclick="return editAdminForm1('<?php echo $this->webroot;?>AdminUsers/update_login')"  value="Submit" class="btn-web btn">
            </div>
...
function show_child(id,ele_type,div_id)
{
    if(document.getElementById(id+div_id).checked === true)
    {
        $("#"+ele_type+id+" input:checkbox").prop("checked", true);
    }
    else
    {
        $("#"+ele_type+id+" input:checkbox").prop("checked", false);
    }
}
