...
function validateExport(url){
    ...
    if(category_type ===""){
        $("#error").html('<span class="w_msg err" style="color:red;">Please select category type.</span>');
        return false;
    }
    else if(fdate ===""){
        ...
        if(url ==="download"){
            $('#validate-form').attr('action','<?php echo $this->webroot;?>MisReports/category_wise');
        }
        if(url ==="view"){
            $('#validate-form').attr('action','<?php echo $this->webroot;?>MisReports/category_reports');
        }
...
<ol class="breadcrumb">                                
    ...
    <li class="active"><a href="#">Category Wise MIS</a></li>
</ol>
<div class="page-heading">            
    <h1>Category Wise MIS</h1>
</div>
