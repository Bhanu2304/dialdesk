...
<ol class="breadcrumb">                                
    ...
    <!-- <li><a href="#">Process Update</a></li> -->
    <li class="active"><a href="#">Process Update</a></li>
</ol>
                    ...
                    <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered datatables" id="exap">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><?php echo $i++; ?></td>
                                <td><?php echo date_format(date_create($post['ProcessUpdation']['date_time']),'d M Y H:i:s'); ?></td>
                                <td><?php echo date_format(date_create($post['ProcessUpdation']['process_date']),'d M Y H:i:s'); ?></td>
                                <td><?php echo $post['ProcessUpdation']['type']; ?></td>
                                <td><?php echo date_format(date_create($post['ProcessUpdation']['request']),'d M Y');?></td>
                                <td><?php echo date_format(date_create($post['ProcessUpdation']['response']),'d M Y');?></td>
                                <td><?php echo date_format(date_create($post['ProcessUpdation']['created_at']),'d M Y');?></td>
                                
                                <td>
                                    <a href="#" onclick="deleteData('<?php echo $this->webroot;?>ProcessUpdates/delete_data?id=<?php echo $post['ProcessUpdation']['id'];?>')" >
