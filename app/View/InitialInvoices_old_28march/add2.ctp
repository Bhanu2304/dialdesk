			...
			<?php // echo $this->Form->create('Add',array('url'=>array('controller'=>'AddInvParticulars','action'=>'index'))); ?>
				...
				<div id="xx"><input type='hidden' value='0' id='par_total' name='par_total'></div>
						...
						<?php  foreach ($tmp_particulars as $post): ?>
							...
							<td><?php echo $this->Form->input('Particular.'.$post['AddInvParticular']['id'].'.qty',array('label'=>false,'readonly'=>$edit_tail,'value'=>$post['AddInvParticular']['qty'],'onkeypress'=>'return isNumberKey(event)')); ?></td>
							<td><?php echo $this->Form->input('Particular.'.$post['AddInvParticular']['id'].'.rate',array('label'=>false,'readonly'=>$edit_tail,'value'=>$post['AddInvParticular']['rate'],'onBlur'=>'getAmount1(this.id)','onkeypress'=>'return isNumberKey(event)')); ?></td>
							<td><?php echo $this->Form->input('Particular.'.$post['AddInvParticular']['id'].'.amount',array('label'=>false,'readonly'=>$edit_tail,'value'=>$post['AddInvParticular']['amount'],'readonly'=>true)); ?></td>
