...
function get_part(particular,rate,qty,amount,initial_id)
{
    ...
    xmlHttpReq.open('POST','add_part?particular='+particular+'&rate='+rate+'&qty='+qty+'&amount='+amount+'&initial_id='+initial_id,true);
    xmlHttpReq.send(null);
			...
			<?php // echo $this->Form->create('Add',array('url'=>array('controller'=>'AddInvParticulars','action'=>'index'))); ?>
				...
				<div id="xx"><input type='hidden' value='0' id='par_total' name='par_total'></div>
						...
						<?php  foreach ($inv_particulars as $post): ?>
							...
							<td><?php echo $this->Form->input('Particular.'.$post['Particular']['id'].'.qty',array('label'=>false,'value'=>$post['Particular']['qty'],'onkeypress'=>'return isNumberKey(event)')); ?></td>
							<td><?php echo $this->Form->input('Particular.'.$post['Particular']['id'].'.rate',array('label'=>false,'value'=>$post['Particular']['rate'],'onBlur'=>'getAmount1(this.id)','onkeypress'=>'return isNumberKey(event)')); ?></td>
