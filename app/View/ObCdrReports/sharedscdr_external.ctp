...
function validateExport(url){
    ...
    if(fdate ===""){
        ...
        if(url ==="download"){
            $('#validate-form').attr('action','<?php echo $this->webroot;?>ObCdrReports/cdr_mis_share_external');
        }
        if(url ==="view"){
            $('#validate-form').attr('action','<?php echo $this->webroot;?>ObCdrReports/sharedscdr_external');
        }
