...
function validateExport(url){
    ...
    if(fdate ===""){
        ...
        if(url ==="download"){
            $('#validate-form').attr('action','<?php echo $this->webroot;?>ObCdrReports/click_to_call_excel');
        }
        if(url ==="view"){
            $('#validate-form').attr('action','<?php echo $this->webroot;?>ObCdrReports/click_to_call');
        }
...
<ol class="breadcrumb">                                
    ...
    <li class="active"><a href="#">CTI OB CDR  Report</a></li>
</ol>
...
<div class="container-fluid">
	...
	<!--<td><?php echo $dt['t3']['DispoSec'];?></td> 
        <td style="text-align: center;" ><a style="text-decoration: none;" href="https://<?php echo $_SERVER[HTTP_HOST];?>/download-recording/download.php?mode=DD&filename=<?php echo $dt['t2']['lead_id'];?>&agent=<?php echo  $dt['t2']['Agent'];?>"><i title="download" class="material-icons">file_download</i></a></td>
