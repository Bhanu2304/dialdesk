...
<div class="srpopup">
    ...
    <div class="divborder">
        ...
        <?php 
        $url= $this->webroot.'AdminsrDetails/update_closeloop';
        $cat1=$close_category[0]['close_cat1'];
        $cat2=$close_category[0]['close_cat2'];
        $srno=$close_category[0]['srno'];
        ?>
        <p class="signin button"> 
            <input type="button" class="btn btn-raised btn-default btn-primary" value="Update" onclick="updateCloseLoop('<?php echo $url;?>','<?php echo $cat1;?>','<?php echo $cat2;?>','<?php echo $srno;?>','<?php echo $cid;?>');"  /> 
        </p>
    </div>	
</div>
