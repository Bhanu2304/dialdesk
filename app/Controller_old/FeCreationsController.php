<?php
class FeCreationsController extends AppController{
    public $helpers = array('Html', 'Form','Js');
    public $components = array('RequestHandler');
    public $uses=array('RegistrationMaster','IvrMaster','PagesMaster','FecreationMaster');
	...
	public function index() {
            $this->layout='user';         
            $result = $this->FecreationMaster->find('all',array('conditions' =>array('create_id' => $this->Session->read('companyid'))));
        ...
        public function view_edit_login(){
            if ($this->request->is('post')) {
                ...
                $user=$this->FecreationMaster->find('first',array('conditions' =>$data));
                $userAss=explode(',',$user['FecreationMaster']['user_right']);
	...
	public function update_login(){
            if ($this->request->is('post')) {
                ...
                $this->FecreationMaster->updateAll($data,array('id'=>$id,'create_id' => $this->Session->read('companyid')));
                echo "1";die;
            }
	}
	...
	public function delete_user(){
		$id  = $this->request->query['id'];
                ...
                /*
                $this->FecreationMaster->query("INSERT INTO `logincreation_master_history` (
		...
		$this->FecreationMaster->delete(array('id'=>$id,'create_id' => $this->Session->read('companyid')));
		$this->redirect(array('action' => 'index'));
	}
	
	public function check_exist_email(){
		$email = $_REQUEST['email'];
		$result = $this->FecreationMaster->find('first',array('conditions' =>array('username' => $email,'create_id'=>$this->Session->read('companyid'))));
		if (!empty($result) ) {
			echo '1';die;
		}
		else{
		...
		$result = $this->FecreationMaster->find('first',array('conditions' =>array('phone' => $phone,'create_id'=>$this->Session->read('companyid'))));
		if (!empty($result) ) {
			echo '1';die;
		}
		else{
		...
		$email = $_REQUEST['getdata'];
		$result = $this->FecreationMaster->find('first',array('conditions' =>array('username' => $email,'id !='=>$_REQUEST['id'],'create_id'=>$this->Session->read('companyid'))));
		if (!empty($result) ) {
			echo '1';die;
		}
		else{
		...
		$phone = $_REQUEST['getdata'];
		$result = $this->FecreationMaster->find('first',array('conditions' =>array('phone' => $phone,'id !='=>$_REQUEST['id'],'create_id'=>$this->Session->read('companyid'))));
		if (!empty($result) ) {
			echo '1';die;
		}
		else{
    ...
    public function save_login_creation(){
        ...
        if($_REQUEST['name']){
            ...
            if($this->FecreationMaster->save($inputData)){
                ...
                $Sub="Exicom:Fe App Login";
                $EmailText.="Dear $name,<br/><br/>";
                $EmailText.="Your app login account successfully created and your password is-.<br/><br/>";
