<?php
	...
	public $uses=array('ObFieldValue','CampaignName','TmpObData','Obup','ObAllocationMaster','ObCampaignDataMaster','ListMaster','VicidialListMaster');
		...
		if($this->request->is("POST") && !empty($this->request->data)){
			...
			if(($FileTye=='application/csv' || $FileTye=='application/vnd.ms-excel' || $FileTye=='application/octet-stream') && strtolower(end($info)) == "csv"){
				...
				if (($handle = fopen($csv_file, "r")) !== FALSE) {
					...
					'TotalCount'=>count($filedata),'list_id'=>$listid,'upload_type'=>$uploadType);
