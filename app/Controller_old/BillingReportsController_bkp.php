...
class BillingReportsController extends AppController{
	...
	public $uses=array('RegistrationMaster','PlanMaster','Waiver','BalanceMaster','BillingMaster','BillMasterPost');
	
    public function beforeFilter() 
    {
        parent::beforeFilter();
	$this->Auth->allow('index','view_bill','view_invoice','view_statement','update_client_plan','get_client_name','allocate_plan','getbillmonth');
    ...
    {
        $this->layout = "user";;
        $clientIds = array_values($this->BalanceMaster->find('list',array('fields'=>array('Id','clientId'),'conditions'=>array('not'=>array('PlanId'=>null)))));
    ...
    public function view_invoice() 
    {
        ...
        $postBill = $this->BillMasterPost->query("SELECT Id,clientId,DATE_FORMAT(BillStartDate,'%d %b %Y') `BillStartDate`,DATE_FORMAT(BillEndDate,'%d %b %Y') `BillEndDate`, LastBilled, paymentDue,
