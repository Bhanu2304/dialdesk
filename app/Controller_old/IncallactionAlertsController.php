<?php
class IncallactionAlertsController extends AppController{
    
    public $helpers = array('Html', 'Form','Js');
    public $components = array('RequestHandler');
    public $uses=array('Escalation','EscalationValue','ClientCategory','LogincreationMaster','FieldCreation','Crone','RegistrationMaster','Matrix','SMSText','TimePeriod','CloseLoopMaster','InCallActionMatrix','InCallActionSms');
    ...
    public function beforeFilter() {
        ...
        {
            $this->Auth->allow('index','add','view','view_value','delete','getParent','view_fields','matrix','getEcr','smsText','smsTextCustomer','time_period','save_incall_action_matrix','delete_matrix','save_customer_smstext','delete_sms','save_smstext','view_edit_data','update_incall_action_matrix','get_edit_ecr','update_customer_smstext','call_action_subtype','get_callaction_typename');
	}
	else
	{
    ...
    public function call_action_subtype(){
        $ClientId=$this->Session->read('companyid');
        if(isset($_REQUEST['id']) && $_REQUEST['id'] !=""){
            $ParentList=$this->CloseLoopMaster->find('list',array('fields'=>array('id','close_loop_category'),'conditions'=>array('parent_id'=>$_REQUEST['id'],'close_loop'=>'manual','client_id'=>$ClientId)));
            echo "<option value=''>Call Action Sub Type</option>";
    ...
    public function view_fields(){
        ...
        $ParentList=$this->CloseLoopMaster->find('list',array('fields'=>array('id','close_loop_category'),'conditions'=>array('parent_id'=>NULL,'close_loop'=>'manual','client_id'=>$ClientId)));
        ...
        $this->set('data1',$this->InCallActionMatrix->find('all',array('conditions'=>array('alertType'=>'Alert','clientId'=>$ClientId))));
        
        $this->set('data2',$this->Matrix->find('all',array('conditions'=>array('alertType !='=>'Alert','clientId'=>$ClientId))));
        $this->set('data3',$this->InCallActionSms->find('all',array('conditions'=>array('clientId'=>$ClientId,'sendType'=>'0'))));
     ...
     public function view_edit_data(){
        ...
        if(isset($_REQUEST['id'])){
            if($_REQUEST['type'] ==="tab1"){ 
               $data1=$this->InCallActionMatrix->find('first',array('conditions'=>array('id'=>$_REQUEST['id'],'clientId'=>$ClientId)));
               $this->set('data1',$data1['InCallActionMatrix']);
            }
            ...
            if($_REQUEST['type'] ==="tab3"){ 
               $data2=$this->InCallActionSms->find('first',array('conditions'=>array('id'=>$_REQUEST['id'],'clientId'=>$ClientId,'sendType'=>'0')));
               $this->set('data2',$data2['InCallActionSms']);
            } 
    ...
    public function save_incall_action_matrix(){
        if($this->request->is('POST')){
            $ClientId = $this->Session->read('companyid');
            $data = $this->request->data['IncallactionAlerts'];
            $data['clientId']=$ClientId;
            $data['call_action_type_name']=$this->get_callaction_typename($data['call_action_type'],$ClientId);
            $data['call_action_sub_type_name']=$this->get_callaction_typename($data['call_action_sub_type'],$ClientId);
            $data['createdate'] = date('Y-m-d H:i:s');
            
            if($this->Session->read("role") =="client"){
                $data['createby'] ='Client - '.$this->Session->read("email");
            }
            else if($this->Session->read("role") =="agent"){
                $data['createby'] ='Client - '.$this->Session->read("agent_username");
            }
            
            $this->InCallActionMatrix->save($data);
            if($data['tabType'] ==="tab1"){
                $this->Session->setFlash("Call Action Alearts Define  SuccessFully.");
            }
            /*
            if($data['tabType'] ==="tab2"){
                $this->Session->setFlash("Esclation Matrix Define  SuccessFully.");
            }*/
            return $this->redirect(array('controller'=>'IncallactionAlerts','action'=>'view_fields','?'=>array('tab'=>$data['tabType'])));
        }  
    }
    
    public function get_callaction_typename($id,$ClientId){
        $name=$this->CloseLoopMaster->find('first',array('fields'=>array('close_loop_category'),'conditions'=>array('id'=>$id,'client_id'=>$ClientId)));
        return $name['CloseLoopMaster']['close_loop_category'];
    }
    ...
    public function update_incall_action_matrix(){
        if($this->request->is('POST')){
            $ClientId = $this->Session->read('companyid');
            $data = $this->request->data['IncallactionAlerts'];
            $data['clientId']=$ClientId;
            ...
            if(isset($data['alertType']) && $data['alertType'] !=""){$dataArr['alertType']="'".$data['alertType']."'";}else{unset($dataArr['alertType']);}
            if(isset($data['call_action_type']) && $data['call_action_type'] !=""){$dataArr['call_action_type']="'".$data['call_action_type']."'";}else{unset($dataArr['call_action_type']);}
            if(isset($data['call_action_sub_type']) && $data['call_action_sub_type'] !=""){$dataArr['call_action_sub_type']="'".$data['call_action_sub_type']."'";}else{unset($dataArr['call_action_sub_type']);}
            if(isset($data['call_action_type_name']) && $data['call_action_type_name'] !=""){$dataArr['call_action_type_name']="'".$data['call_action_type_name']."'";}else{unset($dataArr['call_action_type_name']);}
            if(isset($data['call_action_sub_type_name']) && $data['call_action_sub_type_name'] !=""){$dataArr['call_action_sub_type_name']="'".$data['call_action_sub_type_name']."'";}else{unset($dataArr['call_action_sub_type_name']);}
            if(isset($data['personName']) && $data['personName'] !=""){$dataArr['personName']="'".$data['personName']."'";}else{unset($dataArr['personName']);}
            if(isset($data['designation']) && $data['designation'] !=""){$dataArr['designation']="'".$data['designation']."'";}else{unset($dataArr['designation']);}
            if(isset($data['email']) && $data['email'] !=""){$dataArr['email']="'".$data['email']."'";}else{unset($dataArr['email']);}
            if(isset($data['mobileNo']) && $data['mobileNo'] !=""){$dataArr['mobileNo']="'".$data['mobileNo']."'";}else{unset($dataArr['mobileNo']);}
            if(isset($data['alertOn']) && $data['alertOn'] !=""){$dataArr['alertOn']="'".$data['alertOn']."'";}else{unset($dataArr['alertOn']);}
            if(isset($data['createby']) && $data['createby'] !=""){$dataArr['createby']="'".$data['createby']."'";}else{unset($dataArr['createby']);}
            
           
            if(!empty($dataArr)){
                $this->InCallActionMatrix->updateAll($dataArr,array('id'=>$data['id'],'clientId'=>$ClientId));
            }

            if($data['tabType'] ==="tab1"){
                $this->Session->setFlash("Call Action Alearts Update  SuccessFully.");
            }
            /*
            if($data['tabType'] ==="tab2"){
    ...
    public function update_customer_smstext(){
        if($this->request->is('POST')){
            ...
            if(isset($data['call_action_type_name']) && $data['call_action_type_name'] !=""){$dataArr['call_action_type_name']="'".$data['call_action_type_name']."'";}else{unset($dataArr['call_action_type_name']);}
            if(isset($data['call_action_sub_type_name']) && $data['call_action_sub_type_name'] !=""){$dataArr['call_action_sub_type_name']="'".$data['call_action_sub_type_name']."'";}else{unset($dataArr['call_action_sub_type_name']);}
            if(isset($data['senderID']) && $data['senderID'] !=""){$dataArr['senderID']="'".$data['senderID']."'";}else{unset($dataArr['senderID']);}
            if(isset($data['smsText']) && $data['smsText'] !=""){$dataArr['smsText']="'".$data['smsText']."'";}else{unset($dataArr['smsText']);}
            
           
            
            if(!empty($dataArr)){
                $this->InCallActionSms->updateAll($dataArr,array('id'=>$data['id'],'clientId'=>$ClientId));
            }

            if($data['tabType'] ==="tab3"){
                $this->Session->setFlash("Call Action SMS Text Update SuccessFully.");
            }
            if($data['tabType'] ==="tab4"){
    ...
    public function save_customer_smstext(){
        if($this->request->is('POST')){
            ...
            $this->InCallActionSms->save($data);
            $this->Session->setFlash("Call Action SMS Text Define  SuccessFully.");
            return $this->redirect(array('controller'=>'IncallactionAlerts','action'=>'view_fields','?'=>array('tab'=>$data['tabType'])));
        }  
    }
    ...
    public function delete_matrix(){
        ...
        $this->InCallActionMatrix->deleteAll(array('id'=>$id,'clientId' => $this->Session->read('companyid')));
        $this->redirect(array('controller'=>'IncallactionAlerts','action'=>'view_fields','?'=>array('tab'=>$_REQUEST['tab'])));
    }
    
     public function delete_sms(){
        $id=$this->request->query['id'];
        $this->InCallActionSms->deleteAll(array('id'=>$id,'clientId' => $this->Session->read('companyid')));
        $this->redirect(array('controller'=>'IncallactionAlerts','action'=>'view_fields','?'=>array('tab'=>$_REQUEST['tab'])));
    }
