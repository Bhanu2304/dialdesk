...
function go_to_next3(){
    ...
    if(uploadArray.length ==0) {
        ...
        $("#step3_msgbox").html('Please upload any one documents.');
        return false;
    }
    else if(!terms_condition.checked){
        $("#step3_box").show();
        document.getElementById("step3_box").className = "error";
        $("#step3_msgbox").html('Terms & Condition is required.');
            
            $("#thide").after('<span class="w_msg" style="color:red;">Terms & Condition is required.</span>');
    }
    else{
            $('#ckloder_submit').show();
