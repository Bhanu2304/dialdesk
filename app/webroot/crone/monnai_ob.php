...
$fields_qry    =   mysql_query("SELECT client_id,field,map_field FROM master_fields_map where type='ob' $where_get_tag order by client_id");
...
foreach ($fields_and_values as $client_id => $field_value_pairs)
{
    ...
    $check_back_data    =   mysql_query("SELECT client_id FROM master_data where type='ob' and client_id='$client_id' limit 1");
    ...
    #echo "SELECT $selectColumns FROM call_master_out WHERE  $whereClause ";echo "<br>";
        
    $call_master = mysql_query("SELECT $selectColumns FROM call_master_out  $whereClause ");
    while($master=mysql_fetch_assoc($call_master))
    {
        ...
        if($insert_sql)
        {
            mysql_query("UPDATE `call_master_out` SET OtherStatus='1' WHERE ClientId='$client_id' AND Id='$data_id'");
        }
    }
