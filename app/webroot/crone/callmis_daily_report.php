...
$select = "SELECT * FROM reportmatrix_master WHERE IF(report_type='daily' AND CONCAT(HOUR(NOW()),':',MINUTE(NOW()))=report_value,TRUE,FALSE)";

//$select = "SELECT * FROM reportmatrix_master WHERE id='12' AND client_id='281'";
...
while($row = mysql_fetch_assoc($export)){
    ...
    if($row['report']=='Call MIS'){
        call_mis($condition1,$row['client_id'],$camaign,$condition3,$row['user_name'],$row['user_email'],$db1,'daily');
    }    
}
