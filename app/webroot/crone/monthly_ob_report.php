...
$select = "SELECT * FROM obreportmatrix_master WHERE IF(report_type='monthly' AND CURDATE() = DATE(report_value),TRUE,FALSE)";
...
while($row = mysql_fetch_assoc($export)){
    ...
    if($row['report']=='Time Wise MIS'){
        time_mis($condition1,$row['client_id'],$camaign,$condition2,$name,$email,$db1,'monthly');
    }
    else if($row['report']=='Agent Wise MIS'){
        agent_mis($condition1,$row['client_id'],$camaign,$condition2,$name,$email,$db1,'monthly');
    }
    else if($row['report']=='Abend Call MIS'){
        aband_mis($condition1,$row['client_id'],$camaign,$condition2,$name,$email,$db1,'monthly');
    }
    else if($row['report']=='Answer Call MIS'){
        answer_mis($condition1,$row['client_id'],$camaign,$condition2,$name,$email,$db1,'monthly');
    }
    else if($row['report']=='Category Wise MIS'){
        category_mis($condition1,$row['client_id'],$camaign,$condition2,$name,$email,$db1,'monthly');
    }
    else if($row['report']=='Escalation Level MIS'){
        esc_level_mis($condition1,$row['client_id'],$camaign,$condition2,$name,$email,$db1,'monthly');
    }
    else if($row['report']=='In Call Details MIS'){
        incall_details_mis($condition1,$row['client_id'],$camaign,$condition3,$name,$email,$db1,'monthly');
    }
}
