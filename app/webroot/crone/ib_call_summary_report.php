...
$select = "select * from scenario_automate where report_type='call_summary_in'";
     ...
     FROM asterisk.`vicidial_closer_log` t2
        ...
        $call_list = array();
            ...
            foreach($dialer_arr as $dial)
            {
                ...
                if(!in_array($phone,$call_list[$call_date]))
                {
                    $call_list_summary[$call_date][$calltype] +=1;
                }

                $call_list[$call_date][]=array('call_date'=>$dial['call_date'],'calltype'=>$calltype,'phone'=>$phone);
                $total_no_dial +=1;
            }
            ...
            //print_r($call_list_summary);exit;
                ...
                $tag_arr = mysqli_query($db2,"SELECT *,date(CallDate) call_date FROM call_master cm WHERE clientId='$client_id' and $condition2");
          ...
          //print_r($campaign_list);exit;  
            foreach($call_date_list as $call_date)
            {
               ...
               // {
                    ...
                    $conn_calls = $call_list_summary[$call_date]['Connected'];
                    //print_r($conn_calls);exit;
                    $objWorksheet->setCellValue("A{$row}","All");
                ...
                //{
                    ...
                    $grand_total = 0;
                    foreach($tag_list[$call_date] as $scen=>$tag)
                    {
            ...
            foreach($call_date_list as $call_date)
            {
                ...
                //foreach($campaign_list as $campaign){
                    foreach($call_list[$call_date] as $call_det)
                    {
           ...
           /*$fileName = "abcdef".date("d-m-Y") . '.xlsx';
            ...
            $objWriter->setIncludeCharts(TRUE);
            $objWriter->save('php://output');
            $filename = "/var/www/html/dialdesk/app/webroot/crone/csv_data/dialdesk_ib_call_summary_".date('d_m_Y_H_i_s')."_Export.xls";
