...
$select = "SELECT * FROM crone_job where clientId='523' and cus_email!='' and cus_email is not null and cus_status is null group by data_id;";
...
while($row = mysql_fetch_assoc($export))
{
  ...
  *::before, *::after {
  ...
  $html.= "
        ...
        <div style='background-color:#607d8b;color:#fff;width:100%;font-size:25px;text-indent:5px;'>Ticket Details</div>
 ...
 //echo $html; die;
  ...
  //print_r($smsText);
    ...
    {  
      ...
      $Subject="Ticket ".$Clientname['company_name']." ".$data['In Call ID'];
      $EmailText .=$html;
      ...
      try
      {
          ...
          $updQry = "update crone_job set cus_status='$done',mail_date=now() where data_id = '{$row['data_id']}'";
          mysql_query($updQry);
  ...
  CallFrom='{$dataEmpty['No']}',Duration='1',Unit='1',Amount='0',DedType='Email',DedSubType='Alert'";  
    ...
    if($row['alertOn']=='sms' || $row['alertOn']=='both')
    {
      ...
      {
       $done = send_sms($num);  
       $smsText_sl = addslashes($smsText);
       $updQry = "update crone_job set msg='$smsText_sl',sms_status='$done',sms_date=now() where data_id = '{$row['data_id']}' and id='{$row['id']}' ";
       mysql_query($updQry);
      ...
      $updQry2 = "insert into sms_log(clientId,data_id,alertType,sms_status,sms_date,processType) "
           ...
           {
       $ins = "INSERT INTO billing_master SET clientId='{$row['clientId']}', data_id='{$row['data_id']}',CallDate=CURDATE(),CallTime=TIME(NOW()),SrNo='{$dataEmpty['ID']}',
   CallFrom='{$dataEmpty['No']}',AlertTo='{$row['mobileNo']}',Duration='$strlen',Unit='$unit',Amount='0',DedType='SMS',DedSubType='Alert'";    
