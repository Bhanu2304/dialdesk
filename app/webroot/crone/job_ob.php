...
$select = "SELECT * FROM crone_job WHERE bpo='1' and
IF(alertOn='email' && mail_status IS NULL,TRUE,
 IF(alertOn='sms' && sms_status IS NULL,TRUE,
  IF(alertOn='both' && mail_status IS NULL && sms_status IS NULL,TRUE,FALSE))) && alertType='Alert' and clientId in ('561','469')";
...
while($row = mysql_fetch_assoc($export))
{
  ...
  $callEmpty = "select AllocationId,SrNo `ID`,MSISDN `No`,Category1,Category2,Category3,Category4,Category5 from call_master_out where id={$row['data_id']} limit 1";
    $execallEmpty = mysql_query($callEmpty);
    $dataEmpty = mysql_fetch_assoc($execallEmpty);
    $allocationid = $dataEmpty['AllocationId'];
    $campaignEmpty = "select CampaignId from ob_allocation_name where id={$allocationid} limit 1";
    $execampaignEmpty = mysql_query($campaignEmpty);
    $campEmpty = mysql_fetch_assoc($execampaignEmpty);
    $CampaignId = $campEmpty['CampaignId'];
...
//print_r($fields); exit;
  ...
  $selectFields = "select FieldName,fieldNumber from obfield_master where ClientId='$clientId' and CampaignId='$CampaignId'";
  ...
  foreach($capture1 as $v)
  {
   ...
   $smsText;   
        ...
        else
        {
            ...
            $selectSrNo = "select SrNo,MSISDN from call_master_out where id='{$row['data_id']}' limit 1";
            $exeSrNo = mysql_query($selectSrNo);
            $srno = mysql_fetch_assoc($exeSrNo);
 ...
 $selectCustomerData = "select $fields3 from $table where id='{$row['data_id']}' limit 1";
  ...
  $capture = "SELECT GROUP_CONCAT(CONCAT('field',fieldNumber,'`',FieldName,'`'))`fields` FROM `obfield_master` WHERE `ClientId` = '{$row{'clientId'}}' and CampaignId='$CampaignId' and FieldStatus is null ORDER BY Priority ASC limit 1";
    $execapture = mysql_query($capture);
    $fields = mysql_fetch_assoc($execapture);
    
    $call = "select SrNo `In Call ID`,MSISDN `Call From`,Category1 `Scenario`,Category2 `Sub-Scenario1`,Category3 `Sub-Scenario2`,Category4 `Sub-Scenario3`,Category5 `Sub-Scenario4`,".$fields['fields']." from call_master_out where id={$row['data_id']} limit 1";
