...
//mysql_connect('192.168.137.230','root','vicidialnow',false,128);
mysql_select_db('db_dialdesk') or die('unable to connect');


$select = "SELECT * FROM incall_action_crone_job WHERE
IF(alertOn='email' && mail_status IS NULL,TRUE,
 IF(alertOn='sms' && sms_status IS NULL,TRUE,
  IF(alertOn='both' && mail_status IS NULL && sms_status IS NULL,TRUE,FALSE))) && alertType='Alert'";


//$select = "SELECT * FROM incall_action_crone_job WHERE id='4675'";
...
while($row = mysql_fetch_assoc($export)){
    ...
    else
    {
        if($row['clientId']=='')
        {
          ...
          //$fields = explode('#',$smsText);
            ...
            //print_r($fields); exit;
              ...
              $selectFields = "select FieldName,fieldNumber from close_field where ClientId='$clientId' AND FieldStatus IS NULL";
        ...
        else
        {
          ...
          $smsText .= $row['msg'];
            ...
            $smsText = str_replace(":SrNo:",$srno_, $smsText);
  ...
  for($i=0; $i<count($captureFields); $i++)
  {
      ...
      //print_r($fields); exit;
      if(in_array(trim($captureFields[$keys[$i]]),$fields))
      {
          if($flag) $capture .=",";
          
          $capture .= 'cfield'.$keys[$i];
          $capture1[] = "##{$keys[$i]}#";
          $smsText = str_replace(":".trim($captureFields[$keys[$i]]).":", "##{$keys[$i]}#", $smsText);
          $flag = true;
      }
 ...
 $selectCustomerData = "select $fields3 from $table where id='{$row['data_id']}' limit 1";
  ...
  foreach($capture1 as $v)
  {
      $v = str_replace('#', '', $v);
      $v = str_replace(':', '', $v);
      $smsText = str_replace("##$v#",trim($exportData["cfield$v"])."\n", $smsText);
  }
  ...
  $capture = "SELECT GROUP_CONCAT(CONCAT('cfield',fieldNumber,'`',FieldName,'`'))`fields` FROM `close_field` WHERE `ClientId` = '{$row{'clientId'}}' AND FieldStatus IS NULL ORDER BY Priority ASC limit 1";
   ...
   // print_r($fields);die;
    ...
    $call = "select SrNo `In Call ID`,MSISDN `Call From`,Category1 `Scenario`,Category2 `Sub-Scenario1`,Category3 `Sub-Scenario2`,Category4 `Sub-Scenario3`,Category5 `Sub-Scenario4`,".$fields['fields']." from call_master where id={$row['data_id']} limit 1";
...
td, th {
  ...
  for($i=0; $i<count($data); $i++)
  {
    ...
    $callAction  = mysql_query("select CloseLoopCate1,CloseLoopCate2,CloseLoopingDate from call_master where id={$row['data_id']} limit 1");
    $callActArr = mysql_fetch_assoc($callAction);
  
  $html.= "<tr><td>In Call Action Type</td><td>".$callActArr['CloseLoopCate1']."</td></tr>";
  $html.= "<tr><td>In Call Action Sub Type</td><td>".$callActArr['CloseLoopCate2']."</td></tr>";
  $html.= "<tr><td>In Call Action Date</td><td>".$callActArr['CloseLoopingDate']."</td></tr>";
  
  $html.= "</table><br/><br/>"; 
  $html.= "<hr>";
  $html.= "<div style='padding-top:20px;'>For further details kindly login https://dialdesk.in/dialdesk/client_activations/login</div>";
 ...
 //echo $html; die;
  ...
  {  
    ...
    try
    {
        ...
        $updQry = "update incall_action_crone_job set mail_status='$done',mail_date=now() where data_id = '{$row['data_id']}' and id='{$row['id']}'";
        mysql_query($updQry);
...
if($row['alertOn']=='sms' || $row['alertOn']=='both'){
   ...
   if(!empty($row['msg'])){
    $done = send_sms($num);  
    $updQry = "update incall_action_crone_job set sms_status='$done',sms_date=now() where data_id = '{$row['data_id']}' and id='{$row['id']}' ";
    mysql_query($updQry);
   }
