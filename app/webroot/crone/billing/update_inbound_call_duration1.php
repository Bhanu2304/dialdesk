...
$qry=mysql_query("SELECT Id,LeadId FROM billing_master WHERE DedType='Inbound' AND LeadId IS NOT NULL AND (Duration IS NULL OR Duration='')",$db);

while($result=  mysql_fetch_assoc($qry)){
    $CallDu=mysql_query("select length_in_sec from `vicidial_closer_log` where `lead_id` ='{$result['LeadId']}' limit 1",$con);
