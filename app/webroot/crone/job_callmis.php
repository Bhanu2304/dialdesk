...
while($row = mysql_fetch_assoc($export)){
    ...
    {
        call_mis($condition1,$row['client_id'],$camaign,$condition3,$row['user_name'],$row['user_email'],$db1);
    }
...
while($row = mysql_fetch_assoc($export)){
    ...
    $condition3 = "DATE(CallDate) BETWEEN DATE_SUB(CURDATE(),INTERVAL 7 DAY) AND CURDATE() AND DAYNAME(CURDATE()) = '$dayName'";
