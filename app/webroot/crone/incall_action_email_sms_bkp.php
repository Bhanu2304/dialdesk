...
//print_r($fields); exit;
 ...
 $selectCustomerData = "select $fields3 from $table where id='{$row['data_id']}' limit 1";
  ...
  $capture = "SELECT GROUP_CONCAT(CONCAT('field',fieldNumber,'`',FieldName,'`'))`fields` FROM `field_master` WHERE `ClientId` = '{$row{'clientId'}}' ORDER BY Priority ASC limit 1";
...
td, th {
  ...
  for($i=0; $i<count($data); $i++)
  {
    ...
    $callAction  = mysql_query("CloseLoopCate1,CloseLoopCate2,CloseLoopingDate from call_master where id={$row['data_id']} limit 1");
  ...
  $html.= "</table><br/><br/>"; 
  $html.= "<hr>";
  $html.= "<div style='padding-top:20px;'>For further details kindly login http://dialdesk.in/dialdesk/client_activations/login</div>";
