...
$select = "SELECT * FROM crone_job WHERE
IF(alertOn='email' && mail_status IS NULL,TRUE,
 IF(alertOn='sms' && sms_status IS NULL,TRUE,
  IF(alertOn='both' && mail_status IS NULL && sms_status IS NULL,TRUE,FALSE))) && alertType='Alert' && bpo='1' limit 1";
...
while($row = mysql_fetch_assoc($export))
{
  ...
  $smsText = '';$smsText2=''; $i='';$count='';$pos='';$text='';$str='';$fields=''; $captureFields='';
    ...
    $callEmpty = "select SrNo `ID`,MSISDN `No`,Category1,Category2,Category3,Category4,Category5 from call_master_out where id={$row['data_id']} limit 1";
...
//print_r($fields); exit;
 ...
 $selectCustomerData = "select $fields3 from $table where id='{$row['data_id']}' limit 1"; 
  ...
  $capture = "SELECT GROUP_CONCAT(CONCAT('field',fieldNumber,'`',FieldName,'`'))`fields` FROM `obfield_master` WHERE `ClientId` = '{$row{'clientId'}}' ORDER BY Priority ASC limit 1";
...
td, th {
 ...
 //echo $html; die;
  ...
  {  
    ...
    $SenderEmail=array('Email'=>'dialdeskalert@teammas.in','Name'=>'DialDesk'); 
    $ReplyEmail=array('Email'=>'dialdeskalert@teammas.in','Name'=>'DialDesk');
