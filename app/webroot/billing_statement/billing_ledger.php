                ...
                //getting consumption from table
                 ...
                 while($consume = mysqli_fetch_assoc($rsc_consuption_arr))    
                 {
                     ...
                     $hlh_credit = array();
                             ...
                             $html_credit_history[$to_date][] = $hlh_credit;
                 }
                 
                 /*
                  $select_consumption = "SELECT DATE_FORMAT(LAST_DAY(cm_date),'%d-%m-%y') dater,SUM(cm_total) total FROM billing_consume_daily  WHERE client_id='$clientId' and date(cm_date)>='$start_date' and DATE(cm_date) between '$from_date' and '$to_date' GROUP BY YEAR(cm_date),MONTH(cm_date)";
                 $rsc_consuption_arr = mysqli_query($dd,"$select_consumption");
                 //print_r($rsc_consuption_arr);exit;
                 #foreach($rsc_consuption_arr as $consume)
                 while($consume = mysqli_fetch_assoc($rsc_consuption_arr))    
