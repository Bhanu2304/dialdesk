...
$PHP_AUTH_USER='9999';
$PHP_AUTH_PW='pasword@986';
$PHP_SELF=$_SERVER['PHP_SELF'];
if (isset($_GET["query_date"]))				{$query_date=$_GET["query_date"];}
	elseif (isset($_POST["query_date"]))	{$query_date=$_POST["query_date"];}
if (isset($_GET["start_timexx"]))				{$start_timexx=$_GET["start_timexx"];}
	elseif (isset($_POST["start_timexx"]))	{$start_timexx=$_POST["start_timexx"];}	
if (isset($_GET["end_date"]))				{$end_date=$_GET["end_date"];}
	elseif (isset($_POST["end_date"]))		{$end_date=$_POST["end_date"];}
if (isset($_GET["end_timexx"]))				{$end_timexx=$_GET["end_timexx"];}
	elseif (isset($_POST["end_timexx"]))		{$end_timexx=$_POST["end_timexx"];}	
...
$LOGuser_group =			$row[0];
 ...
 $stmt="SELECT allowed_campaigns,allowed_reports,admin_viewable_groups,admin_viewable_call_times from vicidial_user_groups where user_group='ADMIN';"; 
if ($DB) {echo "|$stmt|\n";}
$rslt=mysql_to_mysqli($stmt, $link);
$row=mysqli_fetch_row($rslt);
...
$LINKbase = "$PHP_SELF?query_date=$query_date&start_timexx=$start_timexx&end_date=$end_date&end_timexx=$end_timexx$groupQS$user_groupQS&shift=$shift&show_parks=$show_parks&time_in_sec=$time_in_sec&search_archived_data=$search_archived_data&report_display_type=$report_display_type&DB=$DB";
...
#	echo "\n";
	...
	if ($shift == 'ALL') 
		...
		//if (strlen($time_BEGIN) < 6) {$time_BEGIN = $start_timexx;}
		//if (strlen($time_END) < 6) {$time_END = $end_timexx;}
		}
	if ($shift == '9AM-5PM') 
		{
		$time_BEGIN = "09:00:00";
	...
	if ($show_parks) 
		{
		$park_HEADER=" "._QXZ("PARKS",10)." | "._QXZ("PARK TIME",10)." | "._QXZ("AVG PARK",10)." | "._QXZ("PARKS/CALL",10)." |";
		$park_HEADER_DIV="+----------+------------+------------+------------";
	...
	if ($DB) {#$ASCII_text.= "<tr><td>$park_stmt</td></tr>";
	}
	...
	while($ss_row=mysqli_fetch_row($ss_rslt)) {
		...
		if ($DB) {#$ASCII_text.= "<tr><td>$stmt</td></tr>";
		}
	...
	$stmt="select $userSQL,if(wait_sec>5000,0,wait_sec) wait_sec,talk_sec,dispo_sec,if(pause_sec>5000,0,pause_sec) pause_sec,lead_id,status,dead_sec from ".$agent_log_table." where event_time <= '$query_date_END' and event_time >= '$query_date_BEGIN' $group_SQL $user_group_SQL limit 10000000;";
	$rslt=mysql_to_mysqli($stmt, $link);
	if ($DB) {#$ASCII_text.= "<tr><td>$stmt</td></tr>";
	}
	...
	if ($file_download < 1)
		{
		$ASCII_text .="<tr>";
$ASCII_text .="<th>"._QXZ("USER NAME",25)."</th> <th>"._QXZ("ID",8)."</th> <th>"._QXZ("CALLS",8)."</th> <th>"._QXZ("TIME CLOCK",10)."</th>  <th>"._QXZ("lOGIN TIME",10)."</th> <th>"._QXZ("PARKS",10)."</th><th>"._QXZ("PARK TIME",10)."</th><th>"._QXZ("AVG PARK",10)."</th> <th>"._QXZ("PARKS/CALL",10)."</th> <th> "._QXZ("WAIT",10)." </th> <th> "._QXZ("WAIT %",8)." </th> <th> "._QXZ("TALK",10)." </th> <th> "._QXZ("TALK TIME ",10)." %</th> <th> "._QXZ("DISPO TIME",10)."  % </th> <th> "._QXZ("DISPO",10)."</th> <th> "._QXZ("PAUSE",10)." </th> <th> "._QXZ("PAUSETIME",9)." %</th> <th> "._QXZ("DEAD",10)." </th> <th> "._QXZ("DEAD TIME",9)." % </th> <th> "._QXZ("CUSTOMER",10)." </th> <th> "._QXZ("Login",10)." </th> <th> "._QXZ("Logout",10)." </th> <th> "._QXZ("ACHT",10)." </th>    $sub_statusesHTML";
$ASCII_text .="</tr>";	
		...
		{
$file_output .= _QXZ("USER").","._QXZ("ID").","._QXZ("CALLS").","._QXZ("TIME CLOCK").","._QXZ("LOGIN TIME")."$park_HEADER_CSV,"._QXZ("PARKS").","._QXZ("PARK TIME").","._QXZ("AVG PARK").", "._QXZ("PARKS/CALL").", "._QXZ("WAIT").","._QXZ("WAIT %").","._QXZ("TALK TIME").","._QXZ("TALK %").","._QXZ("DISPO").","._QXZ("DISPOTIME %").","._QXZ("PAUSE").","._QXZ("PAUSETIME %").","._QXZ("DEAD").","._QXZ("DEAD TIME %").","._QXZ("CUSTOMER").","._QXZ("Login").","._QXZ("Logout").","._QXZ("ACHT")."$sub_statusesFILE\n";
		}
	##### END print the output to screen or put into file output variable



     #echo $ASCII_text;exit;
...
$LogoutT = $Logout[0]==''?"00:00:00":"$Logout[0]";
		...
		while ($n < $sub_status_count)
			...
			$$varname=$graph_header."<th class='thgraph' scope='col'>$Sstatus</th>";
		...
		if ($show_parks) 
			...
			$user_holds=	sprintf("%10s", $user_holds); 
			$total_hold_time= sprintf("%10s", $total_hold_time); 
			$avg_hold_time=	sprintf("%10s", $avg_hold_time); 
			$user_hpc=	sprintf("%10s", $user_hpc); 
                  
			$park_AGENT_INFO="  <td>$user_holds </td> <td>$total_hold_time</td> <td>$avg_hold_time</td> <td>$user_hpc</td> ";
		...
		if ($file_download < 1)
			...
			if ($atdr_login_logout_user_link > 0)
				{
				$Toutput = "<tr><td> $Sname[$m] </td><td> $Suser[$m]</td><td> $Scalls[$m] </td><td> $StimeTC[$m]$TCuserAUTOLOGOUT</td><td> $Stime[$m] </td>$park_AGENT_INFO <td>$Swait[$m] </td><td> $Swaitpct[$m] </td><td> $Stalk[$m] </td><td> $Stalkpct[$m] </td><td> $Sdispopct[$m] </td><td> $Sdispo[$m] </td><td> $Spause[$m] </td><td> $Spausepct[$m] </td><td> $Sdead[$m] </td><td> $Sdeadpct[$m] </td><td> $Scustomer[$m] </td><td> $LoginT   </td><td> $LogoutT </td><td>	$ACHt	  </td>    $SstatusesHTML</tr>";
				}
			else
				{
					$Toutput = "<tr><td> $Sname[$m] </td><td> $Suser[$m] </td><td> $Scalls[$m] </td><td> $StimeTC[$m]$TCuserAUTOLOGOUT</td><td> $Stime[$m] </td>$park_AGENT_INFO <td>$Swait[$m] </td><td> $Swaitpct[$m] </td><td> $Stalk[$m] </td><td> $Stalkpct[$m] </td><td> $Sdispopct[$m] </td><td> $Sdispo[$m] </td><td> $Spause[$m] </td><td> $Spausepct[$m] </td><td> $Sdead[$m] </td><td> $Sdeadpct[$m] </td><td> $Scustomer[$m] </td><td> $LoginT   </td><td> $LogoutT   </td><td>    $ACHt	  </td>$SstatusesHTML </tr>";
...
#		echo "$Suser[$m]|$Sname[$m]|$Swait[$m]|$Stalk[$m]|$Sdispo[$m]|$Spause[$m]|$Scalls[$m]\n";
	...
	if ( ($TOT_AGENTS > 0) and (preg_match('/NAME|ID|TIME|LEADS|TCLOCK/',$stage)) )
		...
		while ($m < $k)
			...
			$sort_order[$m] = "$i";
			if ($file_download < 1)
				{$ASCII_text.="<td>"."$TOPsorted_output[$i]"."</td>";}
			else
	...
	if ($show_parks) 
		...
		$park_TOTALS=" <td>$hTOTuser_holds</td>  <td>$hTOTtotal_hold_time</td> <td>$hTOTavg_hold_time</td> <td>$hTOTuser_hpc </td>";
	...
	if ($file_download < 1)
		{
		$ASCII_text.="<tr>";
		$ASCII_text.="<td colspan=\"2\"> "._QXZ("TOTALS",10)." "._QXZ("AGENTS",10,"r").":$hTOT_AGENTS           </td><td> $hTOTcalls </td><td> $hTOTtimeTC </td><td> $hTOTALtime </td>$park_TOTALS  <td>$hTOTwait</td> <td> $hTOTwaitpct </td><td> $hTOTtalk </td><td>$hTOTtalkpct </td><td>$hTOTdispo </td><td> $hTOTdispopct </td><td> $hTOTpause </td><td> $hTOTpausepct </td><td> $hTOTdead </td><td> $hTOTdeadpct </td><td> $hTOTcustomer </td><td> 00:00:00    </td><td>00:00:00    </td><td>00:00:00    </td>   $SUMstatusesHTML\n";
		$ASCII_text.="</tr>";
		if ($AUTOLOGOUTflag > 0)
			{
...
if ($report_display_type=="HTML")
	...
	// It will be called LIST_101_20090209-121212.txt
	header("Content-Disposition: attachment; filename=\"apr_dialdesk.xls\"");
	header('Expires: 0');
	header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
	header('Pragma: public');
	echo "<table border=\"1\">".$ASCII_text."</table>"; exit;
	#$records = explode("|",$ASCII_text);
