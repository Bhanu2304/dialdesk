...
//ispark connection
$ispark_host       =   "103.182.153.91"; 
$ispark_user   =   "root";
$ispark_pass   =   "321*#LDtr!?*";
$ispark_name    =   "db_bill";
    ...
    while($data = mysqli_fetch_assoc($send_mail))
    {
        ...
        while($risk = mysqli_fetch_assoc($rsc_risk))
        {
                ...
                if($PlanId !="")
                {
                    ...
                    $table_header  = "<table border=\"2\"><tr><th>Client Name</th><th>Plan Mode</th><th>Subscription Value</th><th>Credit Value</th><th>Risk %</th><th>Trigger Usage %</th></tr>";
                    $ClientRsc = mysqli_query($dd,"SELECT * FROM registration_master rm 
                    INNER JOIN  balance_master bm ON rm.company_id=bm.clientId
                    INNER JOIN  plan_master pm ON bm.PlanId = pm.Id
                    WHERE rm.status='A' AND bm.start_date IS NOT NULL");
                    ...
                    while($ClientMaster = mysqli_fetch_assoc($ClientRsc))
                    {
                        ...
                        {
                            $send_email = true;
                            $table_trigger_detail[]  = "<tr><td>".$ClientMaster['company_name']."</td>";
                            
                             $table_trigger_detail[] .= "<td>".$ClientMaster['PeriodType']."</td>";
                             $table_trigger_detail[] .= "<td>".$ClientMaster['RentalAmount']."</td>";
                             $table_trigger_detail[] .= "<td>".$ClientMaster['CreditValue']."</td>";
                             $table_trigger_detail[] .= "<td>".$percent."</td>";
                             $table_trigger_detail[] .= "<td>".$consume_percent."</td></tr>";
                            ...
                            // $emaildata['Attachment'] = $file;
                            // echo $emaildata['Attachment'];die;
                            if($send_email)
                            {
