...
$ispark = mysql_connect("$ispark_host", "$ispark_user", "$ispark_pass")or die("Ispark Server Connection Failed"); 
...
include("ExposureViews2Controller.php");
...
if($BalanceMaster['PlanId'] !=""){
    ...
    while($multitool_det = mysql_fetch_assoc($rsc_multitools_qry))
    {
      ...
      $tool_det = mysql_fetch_assoc($rsc_tool);
    ...
    $free_session_value = addon_credit($ispark,$dd,$dialdee,$clientId,$FromDate,$ToDate);
    #print_r($free_session_value);exit;
    $free_session_remain = 0;
    if($free_session_value>0)
    {
        $free_session_remain = $free_session_value;
    }
   ...
   // old query $InboundDetails=mysql_query("select length_in_sec,queue_seconds,RIGHT(phone_number,10) phone_number,call_date from `vicidial_closer_log` where user !='VDCL' and campaign_id in ($Campagn) AND DATE(call_date) between '$start_date' AND '$ToDate' ",$con);
    //echo "select if(t3.`talk_sec` is null,t2.length_in_sec,t3.`talk_sec`) length_in_sec, phone_number,call_date,t2.user from `vicidial_closer_log` t2 left join vicidial_agent_log t3 on t2.uniqueid=t3.uniqueid where t2.user !='VDCL' and t2.campaign_id in ($Campagn) AND DATE(call_date) between '$FromDate' AND '$ToDate' "; exit;
     $InboundDetails=mysql_query("select if(t3.`talk_sec` is null,t2.length_in_sec,t3.`talk_sec`) length_in_sec, phone_number,call_date,t2.user from `vicidial_closer_log` t2 left join vicidial_agent_log t3 on t2.uniqueid=t3.uniqueid and t2.user=t3.user where t2.user !='VDCL' and t2.campaign_id in ($Campagn) AND DATE(call_date) between '$FromDate' AND '$ToDate' ",$con);
     //$InboundDetails=mysql_query("select t2.length_in_sec length_in_sec, phone_number,call_date,t2.user from `vicidial_closer_log` t2 where t2.user !='VDCL' and t2.campaign_id in ('Vyom','Wiom_OB','Vyom_New_Connection','Vyom_Support','Wiom_OB') AND DATE(call_date) between '$FromDate' AND '$ToDate' ",$con);
     $MultiLangInboundDetails=mysql_query("select if(t3.`talk_sec` is null,t2.length_in_sec,t3.`talk_sec`) length_in_sec, phone_number,call_date,t2.user from `vicidial_closer_log` t2 left join vicidial_agent_log t3 on t2.uniqueid=t3.uniqueid and t2.user=t3.user where t2.user !='VDCL' and t2.campaign_id in ($multilang_ivrs) AND DATE(call_date) between '$FromDate' AND '$ToDate' ",$con);
   ...
   //  echo "select (va.talk_sec-va.dead_sec) length_in_sec,if(LENGTH(phone_number)>'12',LEFT(phone_number,10),RIGHT(phone_number,10)) phone_number,call_date from `vicidial_log` v join vicidial_agent_log va on v.uniqueid=va.uniqueid where length_in_sec!='0' and user !='VDAD' and campaign_id in ($Campagn) AND DATE(call_date) between '$start_date' AND '$ToDate' "; exit;
    $OutboundDetails=mysql_query("select (va.talk_sec-va.dead_sec) length_in_sec, phone_number,call_date,v.user from `vicidial_log` v join vicidial_agent_log va on v.uniqueid=va.uniqueid where length_in_sec!='0' and v.user !='VDAD' and v.campaign_id in ($Campagn) AND DATE(call_date) between '$FromDate' AND '$ToDate' ",$con);
    ...
    $SMSDetails = mysql_query("SELECT DATE_FORMAT(CallDate,'%d %b %y') `CallDate1`,CallDate,CallTime,CallFrom,Unit,AlertTo FROM `billing_master` WHERE clientId='$clientId' AND DedType='SMS' AND date(CallDate) between '$FromDate' AND '$ToDate'");
   ...
   // $Social_WhatsappDetails = mysql_query($Social_Whatsapp_Qry,$dialdee);
    ...
    $Social_bot_Qry = "select date(created_at) CallDate,time(created_at) CallTime,customer_no CallFrom,session_id from chat_customer where client_id='$clientId' and date(created_at) between '$FromDate' and '$ToDate'";  
    $Social_botDetails = mysql_query($Social_bot_Qry,$dialdee);
    ...
    foreach($OutData_aband as $call_date=>$inb_arr)
    {
        ...
        $html2_ab .="<tr><td colspan='4' ><b>Total</b></td>";
   ...
   if(mysql_num_rows($MultiLangOutboundDetails) > 0){
    ...
    if(!empty($Botapp['Unit'])) {
        $bunit = 0;
        if($Botapp['Unit']>=$free_session_remain)
        {
            $bunit = $Botapp['Unit'] - $free_session_remain;
        }
        else if(!empty($Botapp['Unit']))
        {
            $bunit = 0;
        }
        $TBoatAmount=round($bunit*$inbot_charge,2);
   ...
   $html .="<table><tr><td>&nbsp;</td></tr></table>";
    ...
    if(!empty($free_session_remain))
    {
        $html .="<tr>";
        $html .="<td>Free Session</td>";
        $html .="<td>$free_session_remain</td>";
        $html .="<td> Free</td>";
    ...
    if(!empty($Botapp['Unit'])) {
        ...
        if($Botapp['Unit']>=$free_session_remain)
        {
            $Botapp['Unit'] = $Botapp['Unit'] - $free_session_remain;
        }
        else if(!empty($Botapp['Unit']))
        {
            $free_session_remain -= $Botapp['Unit'];
            $Botapp['Unit'] = 0;
        }
        $html .="<td colspan='2'>".round($Botapp['Unit']*$inbot_charge,2)."</td>";
        $html .="</tr>";
    }
    ...
    $html .="<td></td>";
    $html .="<td></td>";
    $html .="<td colspan='2'>".($TinAmount+$TinAmountNight+$TouAmount+$abTouAmount+$TMinAmount+$TMouAmount+$TvfAmount+$TsmAmount+$TemAmount+$TivAmount+$TWhatsAppAmount+$TBoatAmount-$waiver)."</td>";
    $html .="</tr>";
...
echo $html.$html1.$html1N.$html2.$html2_ab.$htmlMulti.$htmlMultiOB.$html3.$html4.$html5.$html6.$htmlWhatsapp.$htmlBotapp ;die;
