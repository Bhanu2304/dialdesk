...
function get_closing_bal($clientId,$dd,$ispark)
{
    ...
    where ti.cost_center='$cost_center' and date(bpp.pay_dates)  between '$from_date' and '$to_date' ";    
                ...
                $waiver_qry = "SELECT SUM(Balance) bal FROM `waiver_master` WHERE clientId='$clientId' limit 1";
                $rsc_waiver = mysqli_query($dd,$waiver_qry);
                $waiver = mysqli_fetch_assoc($rsc_waiver);
                ...
                $data[$clientName]['op2_credit'] = ($op2_credit + $waiver_bal);
...
$select_risk = "SELECT bre.* FROM `billing_risk_exposure` bre 
INNER JOIN `registration_master` rm ON bre.client_id = rm.company_id
WHERE  bre.risk_action='email' AND rm.status !='CL'";
