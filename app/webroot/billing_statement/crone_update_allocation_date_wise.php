...
$activeplan_qry = "SELECT * FROM `billing_plan_alloc_log` WHERE  `status`='1' "; 
$active_date_rsc = mysqli_query($dd,$activeplan_qry);

while($active_det = mysqli_fetch_assoc($active_date_rsc))
{
    $client_id = $active_det['client_id']; 
    $start_date = $active_det['start_date'];
    $day_before = date('Y-m-d',strtotime("-1 days"));
     $last_date = $start_date; 
    $active_id = $active_det['id'];
    while(strtotime($last_date)<=strtotime($day_before))
    {
        $todays_day_is_one = date('d',strtotime($last_date));
        $end_date_of_month = date('Y-m-t',strtotime($last_date));
        $fin_year = date('Y',strtotime($last_date));
        $fin_month = date('M',strtotime($last_date));

        $selec_all_clients_qry = "SELECT * FROM `registration_master` WHERE company_id='$client_id' and `status`='A' "; #`status` in ('A','H','D') 
        ...
        if($BalanceMaster['PlanId'] !=""){
            ...
            $MultiLangInboundQry = "select if(t3.`talk_sec` is null,t2.length_in_sec,t3.`talk_sec`) length_in_sec, phone_number,call_date from `vicidial_closer_log` t2 left join vicidial_agent_log t3 on t2.uniqueid=t3.uniqueid where t2.user !='VDCL' and t2.campaign_id in ($multilang_ivrs) AND DATE(call_date) = '$last_date'";
        ...
        //        }
            ...
            $delete_old = "delete from billing_consume_daily where  client_id='$clientId' and date(cm_date)='$last_date' ";
            $rsc_old = mysqli_query($dd,$delete_old);
      ...
      echo  $last_date = date('Y-m-d',strtotime($last_date." +1 days")); 
    }
    
    $upd_status_log = "update billing_plan_alloc_log set status='0' where id='$active_id'";
    $rsc_status_log = mysqli_query($dd,$upd_status_log);
}
