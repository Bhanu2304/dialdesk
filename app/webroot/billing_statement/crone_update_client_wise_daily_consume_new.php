...
mysql_select_db("$db_name",$dialdee)or die("cannot select DB");
 ...
 $selec_all_clients_qry = "SELECT * FROM `registration_master` WHERE  `status`='A' "; #`status` in ('A','H','D') #company_id='301' and
...
while($ClientInfo = mysqli_fetch_assoc($RegistrationMaster))
{
    ...
    $ib_multi_pulse = 0;      $ib_multi_secs=0;         $ib_multi_charge = 0;     $ib_multi_flat = 0;       $ib_multi_total = 0;      $ib_multi_pulse_per_call = 60;
    $ob_pulse = 0;      $ob_secs=0;         $ob_charge = 0;     $ob_flat = 0;       $ob_total = 0;      $ob_pulse_per_call = 60;
    $ob_multi_pulse = 0;      $ob_multi_secs=0;         $ob_multi_charge = 0;     $ob_multi_flat = 0;       $ob_multi_total = 0;      $ob_multi_pulse_per_call = 60;
...
if($BalanceMaster['PlanId'] !=""){
    ...
    $ifmp_multi = ceil(60/$ib_multi_pulse_sec);
    ...
    $ib_multi_charge = $PlanDetails['MultiIBCharges'];
    $ob_charge = $PlanDetails['OutboundCallCharge'];
    $ob_multi_charge = $PlanDetails['MultiOBCharges'];
    ...
    $MultiLangInboundQry = "select if(t3.`talk_sec` is null,t2.length_in_sec,t3.`talk_sec`) length_in_sec, phone_number,call_date from `vicidial_closer_log` t2 left join vicidial_agent_log t3 on t2.uniqueid=t3.uniqueid and t2.user=t3.user where t2.user !='VDCL' and t2.campaign_id in ($multilang_ivrs) AND DATE(call_date) = '$last_date'";
    $MultiLangInboundDetails=mysqli_query($vd,$MultiLangInboundQry);
    ...
    $OutboundQry = "select (va.talk_sec-va.dead_sec) length_in_sec,call_date from `vicidial_log` v join vicidial_agent_log va on v.uniqueid=va.uniqueid where length_in_sec!='0' and v.user !='VDAD' and v.campaign_id in ($Campagn) AND DATE(call_date) ='$last_date' ";
    $OutboundDetails=mysqli_query($vd,$OutboundQry);
    
    $OutboundQry_aband = "SELECT DATE(t2.call_date) AS CallDate,LEFT(t2.phone_number,10) AS PhoneNumber,            
            ...
            FROM asterisk.vicidial_log t2
                        ...
                        WHERE DATE(t2.call_date)= '$last_date' AND t2.campaign_id='dialdesk'
                        AND t2.lead_id IS NOT NULL";
    
    $OutboundDetails_aband=mysqli_query($vd,$OutboundQry_aband);
    
    $MultiLangOutboundQry = "select (va.talk_sec-va.dead_sec) length_in_sec,call_date from `vicidial_log` v join vicidial_agent_log va on v.uniqueid=va.uniqueid where length_in_sec!='0' and v.user !='VDAD' and v.campaign_id in ($multilang_ivrs) AND DATE(call_date) ='$last_date' ";
    $MultiLangOutboundDetails=mysqli_query($vd,$MultiLangOutboundQry);
    ...
    $Social_Whatsapp_Qry = "select created_at,created_at chat_time,customer_no,session_id from chat_customer where client_id='$clientId' and date(created_at)='$last_date'"; 
    $Social_WhatsappDetails = mysqli_query($dd,$Social_Whatsapp_Qry);
    
    
    $Social_bot_Qry = "select created_at,date(created_at) chat_time,contactNo customer_no,session_id from bot_chat where clientId='$clientId' and date(created_at) ='$last_date'";  
    $Social_botDetails = mysql_query($Social_bot_Qry,$dd);

    $Social_bot_Qry2 = "SELECT session_id,customer_no FROM chat_customer WHERE client_id='$clientId' AND DATE(created_at)='$last_date";  
    ...
    while($outDurArr = mysqli_fetch_assoc($OutboundDetails_aband))
    {
        $qry_check_client = "SELECT id FROM aband_call_master acm where ClientId='$clientId' and PhoneNo='".$outDurArr['PhoneNumber']."' and date(Callbackdate)=date('".$outDurArr['CallDate']."') limit 1";
        $rsc_check_client = mysqli_query($dd,$qry_check_client);
        $check_client = mysqli_fetch_assoc($rsc_check_client);
    ...
    while($multi_inbDurArr = mysqli_fetch_assoc($MultiLangInboundDetails))
    {
        $call_duration = $inbDurArr['length_in_sec'];
        $call_pulse = 0;
        $call_rate = 0;
        ...
        {*/
            $convrt_pulse = $call_duration/$ib_multi_pulse_sec;
            //echo 'adf';exit;
            //echo $ib_first_min;exit;
            if($ib_first_min=='1'){
                if($convrt_pulse>$ifmp_multi)
                {
                    ...
                    $call_pulse = $ifmp_multi+ceil($subsequent);
                }
                else
                {
                    $call_pulse = $ifmp_multi; 
                }
                $call_rate = $ib_multi_pulse_rate*$call_pulse;
            }
            else
            {
                $call_pulse = ceil($call_duration/$ib_multi_pulse_sec); 
                $call_rate = $call_pulse*($ib_multi_pulse_rate); 
            }

            $ib_multi_pulse += $call_pulse; 
            $ib_multi_pulse_sec += $call_pulsesec;
            $ib_multi_total += $call_rate;
        //}
        
    }
    
    while($multi_outDurArr = mysqli_fetch_assoc($OutboundDetails))
    {
        $callLength = round($outDurArr['length_in_sec']);  
        if(!empty($callLength))
        {
            $call_rate = 0;
            $convrt_pulse = $callLength/$ob_multi_pulse_sec; 
            if($ob_first_min=='1'){
                    if($convrt_pulse>$ofmp_multi)
                    {
                     $subsequent = ceil($convrt_pulse-$ofmp_multi); 
                     $total_pulse = $ofmp_multi+$subsequent;
                    }
                    else
                    {
                     $total_pulse = $ofmp_multi; 
                    }
                    $call_rate = $ob_multi_pulse_rate*$total_pulse;
                }
            else{
                    $total_pulse = ceil($callLength/$ob_multi_pulse_sec);
                    $call_rate = $total_pulse*($ob_multi_pulse_rate);
                }
        }
        

        $ob_multi_pulse += $total_pulse;
        $ob_multi_secs += $call_pulsesec;
        $ob_multi_total += $call_rate;
    }
...
//        }
    ...
    while($misArr = mysqli_fetch_assoc($MissDetails))
    {
        ...
        $miss_rate = ceil($MissUnit*$miss_charge);
        $miss_pulse    += $MissUnit;
        $miss_total      += $miss_rate;
    }
    
    
    $session_check_dialdee = "select * from chat_customer where date(created_at)='$secondlast_date";
    $rsc_session_check = mysqli_query($dd,$session_check_dialdee);
    $session_data = array();
    while($session_check = mysqli_fetch_assoc($rsc_session_check))
    {
        $creation_date = $session_check['created_at'];
        $customer_no = $session_check['customer_no'];
        $session_data[$customer_no][] = $creation_date;
    }
    
    
    $unique_session = array();
    while($whatappArr = mysqli_fetch_assoc($Social_WhatsappDetails))
    {
        $creation_date = $whatappArr['created_at'];
        $customer_no = $whatappArr['customer_no'];
        #$secondlast_date = date('Y-m-d',strtotime("-2 days"));
        #$check_session_exist_time = date('Y-m-d H:i:s',strtotime($creation_date." -1 days"));
        $SocialUnit = 1;
        if( in_array($customer_no,$session_data))
        {
            $time_array = $session_data[$customer_no];
        ...
        if($SocialUnit==1)
        {$unique_session[$customer_no] = $SocialUnit;}
    }
    
        $WhatsappUnit = count($unique_session);
        $social_rate = ceil($WhatsappUnit*$chat_charge);
        $social_pulse    += $WhatsappUnit;
        $social_total      += $social_rate;
        
    $session_check_bot = "select * from chat_customer where clientid='$clientId' and date(created_at)='$secondlast_date";
    $rsc_session_bot = mysqli_query($dd,$session_check_bot);
    ...
    $cm_total = round($ib_total+$ibn_total+$ob_total+$ib_multi_total+$ob_multi_total+$ivr_total+$miss_total+$vfo_total+$sms_total+$email_total+$social_total,3);

  echo  $insert_qry = "INSERT INTO `billing_consume_daily` SET client_id='$clientId',cm_date='$last_date',cm_total='$cm_total',";
    ...
    $insert_qry .= "ib_multi_pulse='$ib_multi_pulse',";       $insert_qry .= "ib_multi_secs='$ib_multi_secs',";     $insert_qry .= "ib_multi_charge='$ib_multi_charge',";     $insert_qry .= "ib_multi_flat='$ib_multi_flat',";     $insert_qry .= "ib_multi_total='$ib_multi_total',";
    ...
    $insert_qry .= "ob_multi_pulse='$ob_multi_pulse',";         $insert_qry .= "ob_multi_secs='$ob_multi_secs',";       $insert_qry .= "ob_multi_charge='$ob_multi_charge',";       $insert_qry .= "ob_multi_flat='$ob_multi_flat',";       $insert_qry .= "ob_multi_total='$ob_multi_total',";
    ...
    $insert_qry .= "botapp_pulse='$bot_social_pulse',";   $insert_qry .= "botapp_charge='$inbot_charge',"; $insert_qry .= "botapp_flat='0',"; $insert_qry .= "botapp_total='$bot_social_total',";
            $insert_qry .= "created_at=now(),plan_id='$PlanId'";  exit;
